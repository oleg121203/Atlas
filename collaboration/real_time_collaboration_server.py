...
class CollaborationServer:
            ...
            await self.broadcast(
                {
                    ...
                    "payload": {
                        "user": f"User-{websocket.remote_address[1]}",
                        "status": "online",
                    },
                }
            )
    ...
    async def handle_message(self, message, websocket):
        ...
        try:
            ...
            response = {"action": action, "payload": payload, "timestamp": timestamp}
            await self.broadcast(response, exclude=websocket)

            # Check for conflicts (simplified example)
            if action == "workflow_update" and self.check_for_conflict(payload):
