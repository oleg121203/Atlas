    ...
    def handle_message(self, message):
        ...
        try:
            ...
            if action == "workflow_update":
                print(
                    f"Workflow updated by {payload.get('user')}: {payload.get('workflow_id')}"
                )
                self.on_workflow_update(payload)
            elif action == "presence_update":
                print(
                    f"Presence update: {payload.get('user')} is {payload.get('status')}"
                )
                self.on_presence_update(payload)
            elif action == "conflict_notification":
                print(
                    f"Conflict detected in workflow {payload.get('workflow_id')} by {payload.get('user')}"
                )
