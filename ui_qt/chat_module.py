...
EMOJI_LIST = [":smile:", ":rocket:", ":fire:", ":star:", ":robot:", ":zap:", ":sunglasses:", ":thumbsup:", ":wave:", ":bulb:"]
...
class ChatModule(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.history_btn.setStyleSheet("background: #23272e; color: #00fff7; border-radius: 6px; padding: 4px 12px;")
        ...
        self.chat_history.setStyleSheet("background: #181c20; color: #e0e0e0; border: 1px solid #00fff7; border-radius: 8px; font-size: 15px;")
        ...
        self.input_line.setStyleSheet("background: #23272e; color: #fff; border: 1px solid #00fff7; border-radius: 6px; font-size: 15px;")
        self.input_line.returnPressed.connect(self.send_message)
        input_layout.addWidget(self.input_line, stretch=1)
        self.send_btn = QPushButton(_("Send"))
        self.send_btn.setStyleSheet("background: #00fff7; color: #181c20; font-weight: bold; border-radius: 6px; padding: 6px 18px;")
    ...
    def show_history_dialog(self):
        path, _ = QFileDialog.getOpenFileName(self, "Відкрити історію чату", "", "JSON Files (*.json)")
        if path:
        ...
        if event.type() == QEvent.DragEnter:
            ...
            if event.mimeData().hasUrls():
                for url in event.mimeData().urls():
                    ...
                    self.chat_history.append(f'<span style="color:#ff00a0;">📎 File dropped:</span> {path}')
                    self.append_history("file", path)
                self.scroll_to_bottom()
                return True
            elif event.mimeData().hasText():
                text = event.mimeData().text()
                self.chat_history.append(f'<span style="color:#ff00a0;">📋 Dropped text:</span> {text}')
    ...
    def show_context_menu(self, pos):
        ...
        if action == copy_action:
            ...
            path, _ = QFileDialog.getSaveFileName(self, "Зберегти історію чату", "chat_history.json", "JSON Files (*.json)")
            if path:
