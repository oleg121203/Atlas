...
def check_python_environment():
    ...
    print(
        f"✅ Python версія: {python_version.major}.{python_version.minor}.{python_version.micro}"
    )

    if python_version < (3, 8):
        issues.append(
            f"Python версія {python_version.major}.{python_version.minor} занадто стара. Потрібно 3.8+"
        )
...
def show_diagnostic_results(issues, warnings):
    ...
    if issues:
        ...
        print(
            "   3. Встановіть відсутні залежності: pip install -r requirements-macos.txt"
        )
