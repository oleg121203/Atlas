...
def import_and_check_class(module_path, class_name, description):
    ...
    try:
        ...
        print(f"[{'✓' if class_exists else '✗'}] {description}: {class_name} in {module_path}")
...
def run_protocol_diagnostic():
    ...
    critical_files = [
        ...
        ("docs/reports/security/SECURITY_SYSTEM_REPORT.md.encrypted", "Encrypted Security Report")
    ]
    ...
    #Check classes
    critical_classes = [
        ("agents/encrypted_creator_protocols.py", "EncryptedCreatorProtocols", "Creator Protocol System"),
        ("agents/creator_authentication.py", "CreatorAuthentication", "Creator Authentication System")
    ]
    
    all_classes_exist = True
    for module_path, class_name, description in critical_classes:
        all_classes_exist &= import_and_check_class(module_path, class_name, description)
    
    print("-" * 60)
    print("DIAGNOSTIC SUMMARY")
    print("-" * 60)
    print(f"Essential files status: {'All present' if all_files_exist else 'Some missing'}")
    print(f"Critical classes status: {'All present' if all_classes_exist else 'Some missing'}")
