...
def fix_openai_config():
    ...
    print(
        f"Current OpenAI API Key: {openai_key[:20]}..."
        if len(openai_key) > 20
        else f"Current OpenAI API Key: {openai_key}"
    )
    ...
    is_placeholder = (
        not openai_key
        or openai_key == "YOUR_OPENAI_API_KEY_HERE"
        or openai_key == "sk-your-openai-key-here"
        or openai_key.startswith("sk-placeholder")
    ...
    if is_placeholder:
        ...
        if env_openai_key and not any(
            x in env_openai_key.lower() for x in ["placeholder", "your-", "here"]
        ):
            ...
            if not config.has_option("OpenAI", "note"):
                config.set(
                    "OpenAI",
                    "note",
                    "OpenAI is optional when using Gemini as default provider",
                )
    ...
    if env_path.exists():
        ...
        for line in lines:
            if line.startswith("OPENAI_API_KEY=") and is_placeholder:
                ...
                needs_update = True
            elif line.startswith("DEFAULT_LLM_PROVIDER=") and not line.endswith(
                "gemini"
            ):
    ...
    print("=" * 50)
    print(
        f"• OpenAI API Key: {'Placeholder (safe)' if is_placeholder else 'Configured'}"
    )
    ...
    print(
        f"• Gemini API Key: {'Configured' if config.get('Gemini', 'api_key', fallback='') else 'Missing'}"
    )
    ...
    if is_placeholder:
        ...
        print(
            "   Atlas will continue to work normally with Gemini as the default provider."
        )
        print(
            "   To enable OpenAI features, set a valid OPENAI_API_KEY in your environment."
        )
