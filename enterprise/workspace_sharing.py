...
class WorkspaceSharing:
    ...
    def remove_member(self, workspace_id: str, user_id: str) -> bool:
        ...
        if (
            ...
            or user_id not in self.workspaces[workspace_id]["members"]
        ):
            return False
    ...
    def check_access(
        self, workspace_id: str, user_id: str, resource_id: str, action: str
    ) -> bool:
        ...
        resource_permissions = self.workspaces[workspace_id]["resources"][resource_id][
            "permissions"
        ]
        return (
            user_role in resource_permissions
            and action in resource_permissions[user_role]
        )
    ...
    def setup_routes(self):
        ...
        def create_workspace_route():
            ...
            try:
                ...
                return make_response(
                    jsonify({"error": f"Workspace {workspace_id} already exists"}), 409
                )
            except Exception as e:
                return make_response(
        ...
        def add_member_route(workspace_id):
            ...
            try:
                ...
                if not workspace:
                    return make_response(
                        jsonify({"error": f"Workspace {workspace_id} not found"}), 404
                    )

                if workspace["owner_id"] != decoded["user"] and decoded["role"] not in [
                    "admin",
                    "manager",
                ]:
                    return make_response(
                ...
                if not user_id:
                    return make_response(
                        jsonify({"error": "Missing user_id field"}), 400
                    )

                if self.add_member(workspace_id, user_id, role):
                    return jsonify(
                        {
                            "message": f"User {user_id} added to workspace {workspace_id} as {role}"
                        }
                    )
                return make_response(
                    jsonify({"error": f"Workspace {workspace_id} not found"}), 404
        ...
        @self.app.route(
            "/api/workspaces/<workspace_id>/members/<user_id>", methods=["DELETE"]
        )
        def remove_member_route(workspace_id, user_id):
            ...
            try:
                ...
                return make_response(
                    jsonify(
                        {
                            "error": f"User {user_id} not found in workspace {workspace_id} or cannot be removed"
                        }
                    ),
                    404,
                )
        ...
        @self.app.route(
            "/api/workspaces/<workspace_id>/members/<user_id>/role", methods=["PUT"]
        )
        def update_member_role_route(workspace_id, user_id):
            ...
            try:
                ...
                if self.update_member_role(workspace_id, user_id, new_role):
                    return jsonify(
                        {
                            "message": f"Role updated for user {user_id} in workspace {workspace_id} to {new_role}"
                        }
                    )
                return make_response(
                    jsonify(
                        {
                            "error": f"User {user_id} not found in workspace {workspace_id} or role update not allowed"
                        }
                    ),
                    404,
                )
        ...
        def add_resource_route(workspace_id):
            ...
            try:
                ...
                if self.add_resource(
                    workspace_id, resource_id, resource_type, permissions
                ):
                    return jsonify(
                        {
                            "message": f"Resource {resource_id} added to workspace {workspace_id}"
                        }
                    )
                return make_response(
                    jsonify({"error": f"Workspace {workspace_id} not found"}), 404
        ...
        def check_access_route(workspace_id):
            ...
            try:
                ...
                has_access = self.check_access(
                    workspace_id, decoded["user"], resource_id, action
                )
        ...
        def get_workspace_route(workspace_id):
            ...
            try:
                ...
                if (
                    decoded["user"] not in workspace["members"]
                    and decoded["role"] != "admin"
                ):
                    return make_response(
                        jsonify({"error": "Insufficient permissions"}), 403
                    )
