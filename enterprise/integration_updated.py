...
class EnterpriseIntegration:
    ...
    def log_user_activity(
        ...
        self.activity_tracking.log_activity(
    ...
    def check_user_access(
        ...
        return self.workspace_sharing.check_resource_access(
            workspace_id, user_id, resource_type, resource_id
        )

    def manage_user_role_in_workspace(
        self, workspace_id: str, user_id: str, role: str, action: str, admin_user: str
    ) -> bool:
    ...
    def setup_routes(self):
        ...
        @self.app.route(
            "/api/integrated/user/workspace/<workspace_id>/role", methods=["POST"]
        )
        def manage_user_role_in_workspace_route(workspace_id):
            ...
            try:
                ...
                if (
                    not user_id
                    or not action
                    or (action in ["add", "update"] and not role)
                ):
                    return make_response(
                        jsonify({"error": "Missing required fields"}), 400
                    )

                if self.manage_user_role_in_workspace(
                    workspace_id, user_id, role, action, admin_user
                ):
                    return jsonify(
                        {
                            "message": f"User {user_id} {action}ed in workspace {workspace_id}"
                        }
                    )
                return make_response(
                    jsonify(
                        {
                            "error": f"Failed to {action} user {user_id} in workspace {workspace_id}"
                        }
                    ),
                    403,
                )
        ...
        @self.app.route(
            "/api/integrated/access/<workspace_id>/<resource_type>/<resource_id>",
            methods=["GET"],
        )
        def check_user_access_route(workspace_id, resource_type, resource_id):
            ...
            try:
                ...
                has_access = self.check_user_access(
