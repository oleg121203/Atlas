...
class RealTimeCollaboration:
    ...
    def update_document(self, document_id: str, content: str, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.lock:
            if document_id not in self.documents:
                ...
                self.documents[document_id]['last_updated_at'] = datetime.utcnow().isoformat()
                self.documents[document_id]['history'].insert(0, {
                    'content': old_content,
                    'updated_by': old_user,
                    'updated_at': old_timestamp
                })
                if len(self.documents[document_id]['history']) > 10:  # Limit history to last 10 updates
                    self.documents[document_id]['history'] = self.documents[document_id]['history'][:10]
            self.save_data()
    ...
    def create_task(self, workspace_id: str, task_id: str, title: str, description: str, assigned_to: List[str], created_by: str) -> bool:
    ...
    def update_task_status(self, workspace_id: str, task_id: str, status: str, updated_by: str) -> bool:
        ...
        try:
            async for message in websocket:
                ...
                if action == 'chat':
                    ...
                    sent_message = self.send_chat_message(workspace_id, user_id, chat_message)
                    await self.broadcast_to_workspace(workspace_id, json.dumps({
                        'type': 'chat',
    ...
    def setup_routes(self):
        ...
        def send_chat_message_route(workspace_id):
            ...
            try:
                ...
                chat_message = self.send_chat_message(workspace_id, decoded['user'], message)
        ...
        def update_document_route(document_id):
            ...
            try:
                ...
                updated_doc = self.update_document(document_id, content, decoded['user'])
                if updated_doc:
                    return jsonify({
                        'message': f'Document {document_id} updated',
                        'document': updated_doc
                    })
                return make_response(jsonify({'error': f'Document {document_id} not found'}), 404)
            except Exception as e:
                return make_response(jsonify({'error': f'Invalid token: {str(e)}'}), 401)
        ...
        def create_task_route(workspace_id):
            ...
            try:
                ...
                if self.create_task(workspace_id, task_id, title, description, assigned_to, decoded['user']):
                    return jsonify({'message': f'Task {task_id} created in workspace {workspace_id}'})
                return make_response(jsonify({'error': f'Task {task_id} already exists in workspace {workspace_id}'}), 409)
            except Exception as e:
                return make_response(jsonify({'error': f'Invalid token: {str(e)}'}), 401)

        @self.app.route('/api/collaboration/task/<workspace_id>/<task_id>/status', methods=['PUT'])
        def update_task_status_route(workspace_id, task_id):
            ...
            try:
                ...
                if self.update_task_status(workspace_id, task_id, status, decoded['user']):
                    return jsonify({'message': f'Task {task_id} status updated to {status}'})
                return make_response(jsonify({'error': f'Task {task_id} not found in workspace {workspace_id}'}), 404)
            except Exception as e:
                return make_response(jsonify({'error': f'Invalid token: {str(e)}'}), 401)
