...
class PluginValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.plugin_manager = PluginManager(agent_manager=self.agent_manager, plugin_dir=self.plugin_dir)
    ...
    def test_plugin_metadata_validation(self):
        ...
        # Validate metadata
        with patch.object(self.plugin_manager, 'get_all_plugins', return_value=self.plugin_manager.plugins):
            ...
            self.assertTrue(valid_result['valid'], "Valid plugin metadata should pass validation")
            self.assertFalse(invalid_result['valid'], "Invalid plugin metadata should fail validation")
            self.assertIn('missing required fields', invalid_result['reason'].lower(), "Validation failure reason should mention missing fields")
    ...
    def test_plugin_security_scan(self):
        ...
        with patch('builtins.open', side_effect=[
            unittest.mock.mock_open(read_data="def safe_function(): pass").return_value,
            unittest.mock.mock_open(read_data="def unsafe_function(): eval('malicious code')").return_value
        ]):
            safe_result = self.plugin_manager._scan_for_security_issues('SafePlugin')
            unsafe_result = self.plugin_manager._scan_for_security_issues('UnsafePlugin')

            self.assertTrue(safe_result['safe'], "Safe plugin should pass security scan")
            self.assertFalse(unsafe_result['safe'], "Unsafe plugin should fail security scan")
            self.assertIn('eval', unsafe_result['issues'][0].lower(), "Security scan should flag use of eval as an issue")
    ...
    def test_plugin_dependency_validation(self):
        ...
        with patch.object(self.plugin_manager, 'get_all_plugins', return_value=self.plugin_manager.plugins):
            ...
            self.assertTrue(valid_result['valid'], "PluginA dependencies should be valid")
            self.assertFalse(invalid_result['valid'], "PluginC dependencies should be invalid due to missing dependency")
            self.assertIn('nonexistentplugin', invalid_result['reason'].lower(), "Validation failure reason should mention missing dependency")
    ...
    def test_plugin_quality_metrics(self):
        ...
        with patch('agents.plugin_manager.run_code_quality_checks', side_effect=[
            ...
            self.assertTrue(good_result['passes'], "Good quality plugin should pass quality checks")
            self.assertFalse(poor_result['passes'], "Poor quality plugin should fail quality checks")
            self.assertIn('high complexity', poor_result['issues'][0].lower(), "Quality check should flag high complexity")
    ...
    def test_plugin_load_without_errors(self):
        ...
        with patch.object(self.plugin_manager, 'get_all_plugins', return_value=self.plugin_manager.plugins):
            ...
            self.assertTrue(success_result['success'], "Plugin should load successfully without errors")
            self.assertFalse(error_result['success'], "Plugin should fail to load due to exception")
            self.assertIn('load error', error_result['error'].lower(), "Load failure reason should include the exception message")
