...
def test_ukrainian_chat_integration():
    ...
    class MockLLMManager:
        def chat(self, messages):
            ...
            if "translate" in system_prompt.lower():
                ...
                elif "допоможи" in content:
                    return MockResult("help me")
                elif "інструменти" in content:
                    return MockResult("tools")
                elif "Hello" in content and "Ukrainian" in system_prompt:
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if chat_context.mode == ChatMode.TOOL_INQUIRY:
            ...
            english_response = "I understand your message. How can I assist you further?"
...
def test_mixed_conversation():
    ...
    for msg, expected_lang, description in conversation:
        detected_lang, confidence = translation_manager.translation_tool.detect_language(msg)
        should_translate = translation_manager.translation_tool.should_translate_message(msg)
        
        status = "✅" if detected_lang == expected_lang else "❌"
        print(f"{status} '{msg}' -> {detected_lang} (confidence: {confidence:.2f}) - {description}")
