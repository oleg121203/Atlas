...
def test_native_screencapture():
    ...
    try:
        ...
        result = subprocess.run(["which", "screencapture"], check=False, capture_output=True, text=True)
...
def test_applescript_capture():
    ...
    try:
        ...
        result = subprocess.run(["which", "osascript"], check=False, capture_output=True, text=True)
        if result.returncode != 0:
