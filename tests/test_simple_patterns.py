...
def test_pattern_detection():
    ...
    mode_patterns = {
        ChatMode.CASUAL_CHAT: {
            'keywords': [
                'hello', 'hi', 'hey', 'привіт', 'привет', 'як справи', 'як тебе звати'
            ]
        },
        ChatMode.SYSTEM_HELP: {
            'keywords': [
                'пам\'ять', 'память', 'memory', 'забезпечена', 'довгострокова', 
                'розмежуванням', 'напрямку', 'чату', 'цікавить', 'мене цікавить'
            ]
        }
    }
    ...
    test_cases = [
        ...
        ("Мене цікавить чи забезпечена в тебе пам'ять довгострокова і з розмежуванням по напрямку чату?", ChatMode.SYSTEM_HELP),
    ...
    for message, expected_mode in test_cases:
        ...
        print(f"   Scores: {[(mode.value, f'{score:.2f}') for mode, score in scores.items()]}")
        print()
...
def test_memory_response_logic():
    ...
    memory_keywords = ['пам\'ять', 'память', 'memory', 'remember', 'memorize', 'store', 'recall', 
                      'забезпечена', 'довгострокова', 'розмежуванням', 'напрямку', 'чату']
    
    detected = any(word in message_lower for word in memory_keywords)
    print(f"Memory keywords detected: {detected}")
    print(f"Detected keywords: {[word for word in memory_keywords if word in message_lower]}")
