...
class TestSlackIntegration(unittest.TestCase):
    ...
    def test_slack_oauth_route(self, mock_get_authorize_url):
        mock_get_authorize_url.return_value = "https://slack.com/oauth/authorize?client_id=test_client_id"
        with self.app.test_request_context('/slack/oauth'):
            response = self.slack_integration.app.route('/slack/oauth')()
            self.assertEqual(response, "https://slack.com/oauth/authorize?client_id=test_client_id")
    ...
    def test_create_task_from_slack(self, mock_send_message):
        ...
        task_id = self.slack_integration.create_task_from_slack("#channel", "Create a task")
    ...
    def test_create_task_from_slack_empty_title(self, mock_send_message):
        ...
        mock_send_message.assert_called_once_with("#channel", "Cannot create a task without a title.")

    @patch('slack_integration.SlackIntegration.send_message')
    @patch('slack_integration.SlackIntegration._retrieve_task')
    @patch('slack_integration.SlackIntegration._store_task')
    def test_update_task_from_slack_complete(self, mock_store_task, mock_retrieve_task, mock_send_message):
        mock_send_message.return_value = True
        mock_retrieve_task.return_value = {"id": "task_123", "title": "Test Task", "status": "open"}
        result = self.slack_integration.update_task_from_slack("#channel", "task_123", "Mark as complete")
        self.assertTrue(result)
        mock_store_task.assert_called()
        mock_send_message.assert_called_once_with("#channel", "Task task_123 marked as completed.")

    @patch('slack_integration.SlackIntegration.send_message')
    @patch('slack_integration.SlackIntegration._retrieve_task')
    @patch('slack_integration.SlackIntegration._store_task')
    def test_update_task_from_slack_note(self, mock_store_task, mock_retrieve_task, mock_send_message):
        mock_send_message.return_value = True
        mock_retrieve_task.return_value = {"id": "task_123", "title": "Test Task", "status": "open"}
        result = self.slack_integration.update_task_from_slack("#channel", "task_123", "Add a note")
        self.assertTrue(result)
        mock_store_task.assert_called()
        mock_send_message.assert_called_once_with("#channel", "Task task_123 updated with note: Add a note")

    @patch('slack_integration.SlackIntegration.send_message')
    @patch('slack_integration.SlackIntegration._retrieve_task')
    def test_update_task_from_slack_not_found(self, mock_retrieve_task, mock_send_message):
        mock_send_message.return_value = True
        mock_retrieve_task.return_value = None
        result = self.slack_integration.update_task_from_slack("#channel", "task_123", "Update task")
        self.assertFalse(result)
        mock_send_message.assert_called_once_with("#channel", "Task task_123 not found.")

    @patch('slack_integration.SlackIntegration.send_message')
    def test_notify_task_progress(self, mock_send_message):
        mock_send_message.return_value = True
        result = self.slack_integration.notify_task_progress("#channel", "task_123", "In progress")
        self.assertTrue(result)
        mock_send_message.assert_called_once_with("#channel", "Task task_123 progress: In progress")
