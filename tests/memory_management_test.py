...
class MemoryManagementTest(unittest.TestCase):
    ...
    def test_memory_manager_initialization(self, mock_process):
        ...
        self.assertEqual(memory_manager.cache_size_limit, 50 * 1024 * 1024)  # 50MB in bytes
        self.assertEqual(memory_manager.cache_ttl.total_seconds(), 600)  # 10 minutes in seconds
        self.assertEqual(memory_manager.get_memory_usage(), 100.0)  # Should return 100MB
    ...
    def test_cache_management(self):
        ...
        self.assertEqual(memory_manager.get_from_cache("key1"), "value1", "Item should be retrievable from cache")
        ...
        self.assertGreater(memory_manager.current_cache_size, 0, "Cache size should be greater than 0")
    ...
    def test_cache_expiration(self, mock_datetime):
        ...
        self.assertEqual(memory_manager.get_from_cache("key1"), "value1", "Item should still be in cache within TTL")

        # Simulate time after TTL
        expired_time = start_time + timedelta(minutes=2)
        mock_datetime.now.return_value = expired_time
        self.assertIsNone(memory_manager.get_from_cache("key1"), "Item should be expired from cache after TTL")
    ...
    def test_memory_management_integration(self, mock_qtimer):
        ...
        self.assertIsNotNone(main_window.memory_manager, "MemoryManager should be initialized in main window")
        ...
        with patch.object(main_window.memory_manager, 'get_memory_usage', return_value=200.0):
            with patch.object(main_window.memory_manager, 'log_memory_stats') as mock_log_stats:
                with patch.object(main_window.memory_manager, 'perform_cleanup') as mock_cleanup:
        ...
        with patch.object(main_window.memory_manager, 'get_memory_usage', return_value=600.0):
            with patch.object(main_window.memory_manager, 'log_memory_stats') as mock_log_stats:
                with patch.object(main_window.memory_manager, 'perform_cleanup') as mock_cleanup:
                    main_window._memory_management_task()
                    mock_log_stats.assert_called_once()
