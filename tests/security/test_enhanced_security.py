...
def test_session_timeout():
    ...
    print(f"   • Сесія активна: {'✅' if status['is_creator_session_active'] else '❌'}")
    print(f"   • Тривалість: {status['session_duration']:.1f} сек")
    print(f"   • Залишилось часу сесії: {timeout_info.get('remaining_session_minutes', 0):.1f} хв")
    ...
    print(f"   • Сесія активна: {'✅' if final_status['is_creator_session_active'] else '❌'}")
...
def test_encrypted_cache_and_logs():
    ...
    print(f"   • Кількість зашифрованих логів: {status.get('encrypted_logs_count', 0)} записів")
...
def test_complete_workflow():
    ...
    if detected == CreatorIdentityLevel.POSSIBLE_CREATOR:
        ...
        if success:
            ...
            auth.store_encrypted_cache("user_context", {
                "mood": "friendly",
            ...
            for i in range(3):
                ...
                auth._log_encrypted_event("USER_INTERACTION", {
            ...
            final_status = auth.get_authentication_status()
            print(f"   • Статус: {'❌ Завершена' if not final_status['is_creator_session_active'] else '✅ Активна'}")
            print(f"   • Кеш очищено: {'✅ Так' if final_status.get('encrypted_cache_size', 0) == 0 else '❌ Ні'}")
