...
class TestUserSatisfactionMonitor(unittest.TestCase):
    ...
    def test_get_current_nps(self):
        ...
        self.assertAlmostEqual(nps, 0.0)  # (1 promoter - 1 detractor) / 3 total * 100 = 0

    def test_get_nps_trend(self):
        self.monitor.collect_nps_score('user1', 'wf1', 9)
        trend = self.monitor.get_nps_trend(days=7)
        self.assertEqual(len(trend), 1)
        self.assertTrue(trend['timestamp'].iloc[0] >= datetime.now() - timedelta(days=7))
    ...
    def test_detailed_feedback_analysis(self):
        self.monitor.collect_feedback('user1', 'wf1', 'Great experience with this workflow')
        self.monitor.collect_feedback('user2', 'wf1', 'Terrible performance, needs improvement')
