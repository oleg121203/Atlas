...
def test_messages():
    ...
    for i, message in enumerate(test_cases, 1):
        ...
        expected = "❓ SYSTEM_HELP" if any(word in message.lower() for word in
                  ["режим", "modes", "atlas", "атлас", "что ты", "що ти", "what is", "help", "explain", "як працює", "как работает"]) else "Other"

        status = "✅" if context.mode == ChatMode.SYSTEM_HELP else "❌"

        print(f'{i:2d}. {status} "{message}"')
        print(f"     Detected: {icon} {context.mode.value.upper()} (confidence: {context.confidence:.2f})")
