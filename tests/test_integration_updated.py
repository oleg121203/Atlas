...
class TestEnterpriseIntegration(unittest.TestCase):
    ...
    def test_log_user_activity(self):
        ...
        try:
            self.integration.log_user_activity(user_id, action, resource_type, resource_id, details)
    ...
    def test_check_user_access_no_membership(self):
        ...
        with patch.object(self.integration.workspace_sharing, 'is_member', return_value=False):
            access = self.integration.check_user_access(user_id, workspace_id, permission, resource_type, resource_id)
    ...
    def test_check_user_access_no_role_permission(self):
        ...
        with patch.object(self.integration.workspace_sharing, 'is_member', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'get_member_role', return_value="user"), \
    ...
    def test_check_user_access_no_resource_access(self):
        ...
        with patch.object(self.integration.workspace_sharing, 'is_member', return_value=True), \
             ...
             patch.object(self.integration.workspace_sharing, 'check_resource_access', return_value=False):
            access = self.integration.check_user_access(user_id, workspace_id, permission, resource_type, resource_id)
            self.assertFalse(access)
    ...
    def test_check_user_access_success(self):
        ...
        with patch.object(self.integration.workspace_sharing, 'is_member', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'get_member_role', return_value="user"), \
             patch.object(self.integration.rbac, 'check_permission', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'check_resource_access', return_value=True):
            access = self.integration.check_user_access(user_id, workspace_id, permission, resource_type, resource_id)
    ...
    def test_manage_user_role_in_workspace_no_admin(self):
        ...
        with patch.object(self.integration.user_management, 'user_exists', return_value=True), \
             patch.object(self.integration.user_management, 'get_user_role', return_value="user"), \
             patch.object(self.integration.rbac, 'check_permission', return_value=False):
            result = self.integration.manage_user_role_in_workspace(workspace_id, user_id, role, action, admin_user)
    ...
    def test_manage_user_role_in_workspace_add_success(self):
        ...
        with patch.object(self.integration.user_management, 'user_exists', return_value=True), \
             patch.object(self.integration.user_management, 'get_user_role', return_value="admin"), \
             patch.object(self.integration.rbac, 'check_permission', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'add_member', return_value=True):
            result = self.integration.manage_user_role_in_workspace(workspace_id, user_id, role, action, admin_user)
    ...
    def test_manage_user_role_in_workspace_add_user_not_exist(self):
        ...
        with patch.object(self.integration.user_management, 'user_exists', side_effect=[True, False]), \
             patch.object(self.integration.user_management, 'get_user_role', return_value="admin"), \
             patch.object(self.integration.rbac, 'check_permission', return_value=True):
            result = self.integration.manage_user_role_in_workspace(workspace_id, user_id, role, action, admin_user)
            self.assertFalse(result)
    ...
    def test_manage_user_role_in_workspace_update_success(self):
        ...
        with patch.object(self.integration.user_management, 'user_exists', return_value=True), \
             patch.object(self.integration.user_management, 'get_user_role', return_value="admin"), \
             patch.object(self.integration.rbac, 'check_permission', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'is_member', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'update_member_role', return_value=True):
            result = self.integration.manage_user_role_in_workspace(workspace_id, user_id, role, action, admin_user)
            self.assertTrue(result)
    ...
    def test_manage_user_role_in_workspace_remove_success(self):
        ...
        with patch.object(self.integration.user_management, 'user_exists', return_value=True), \
             ...
             patch.object(self.integration.rbac, 'check_permission', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'is_member', return_value=True), \
             patch.object(self.integration.workspace_sharing, 'remove_member', return_value=True):
            result = self.integration.manage_user_role_in_workspace(workspace_id, user_id, role, action, admin_user)
            self.assertTrue(result)
