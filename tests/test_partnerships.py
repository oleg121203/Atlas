...
class TestPartnershipManager(unittest.TestCase):
    ...
    def test_add_potential_partner(self):
        partner = self.pm.add_potential_partner("TestCorp", "Tech", "test@corp.com", "High potential")
    ...
    def test_propose_partnership(self):
        partner = self.pm.add_potential_partner("TestCorp", "Tech", "test@corp.com", "High potential")
        proposal = self.pm.propose_partnership(partner['id'], "Integration", "Test integration proposal")
        ...
        updated_partner = next((p for p in self.pm.partners if p['id'] == partner['id']), None)
        self.assertEqual(updated_partner['status'], "Proposal in Progress")

    def test_finalize_proposal(self):
        partner = self.pm.add_potential_partner("TestCorp", "Tech", "test@corp.com", "High potential")
        proposal = self.pm.propose_partnership(partner['id'], "Integration", "Test integration proposal")
        finalized_proposal = self.pm.finalize_proposal(proposal['id'], "Accepted", "Proposal accepted with conditions")
        self.assertEqual(finalized_proposal['status'], "Accepted")
        self.assertEqual(finalized_proposal['comments'], "Proposal accepted with conditions")
    ...
    def test_finalize_agreement(self):
        ...
        self.pm.finalize_proposal(proposal['id'], "Accepted")
        agreement = self.pm.draft_agreement(proposal['id'], "6-month integration", 6)
        finalized_agreement = self.pm.finalize_agreement(agreement['id'], "Active", "Final terms for active partnership")
        self.assertEqual(finalized_agreement['status'], "Active")
        self.assertEqual(finalized_agreement['terms'], "Final terms for active partnership")
    ...
    def test_record_outcome(self):
        ...
        outcome = self.pm.record_outcome(agreement['id'], "Integration Complete", metrics, "Successful integration")
    ...
    def test_get_partnership_summary(self):
        self.pm.add_potential_partner("TestCorp", "Tech", "test@corp.com", "High potential")
