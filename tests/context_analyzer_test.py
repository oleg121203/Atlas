...
class TestContextAnalyzer(unittest.TestCase):
    def setUp(self):
        ...
        self.context_analyzer = ContextAnalyzer(self.memory_manager, context_path="test_context_data")
    ...
    def test_update_user_context_existing_user(self):
        ...
        with patch('datetime.datetime') as mocked_datetime:
            ...
            self.assertEqual(updated_context["interaction_patterns"], {"frequency": "daily", "peak_hours": [9, 10, 11]})
            self.assertEqual(updated_context["current_state"], {"time_of_day": 12, "day_of_week": 2})
            # Check if last_updated is different due to mocked time
            self.assertNotEqual(updated_context["last_updated"], initial_context["last_updated"])
    ...
    def test_get_user_context_existing_user(self):
        ...
        with patch.object(self.context_analyzer, "refresh_current_state") as mock_refresh:
    ...
    def test_refresh_current_state_existing_user(self):
        ...
        self.assertNotEqual(self.context_analyzer.user_context["user_1"]["last_updated"], "2025-06-25T12:00:00")
    ...
    def test_analyze_interaction_patterns_with_interactions(self):
        ...
        interactions = [
            {"timestamp": (datetime.now() - timedelta(days=2)).isoformat(), "query": "hello", "rating": 4},
            {"timestamp": (datetime.now() - timedelta(days=1)).isoformat(), "query": "how are you", "rating": 5},
            {"timestamp": datetime.now().isoformat(), "query": "tell me a joke", "rating": 3}
        ]
        ...
        self.assertTrue(patterns["task_completion_rate"] > 0.5)  # Average rating > 3.5/5
        self.assertIn("last_analysis", patterns)
    ...
    def test_infer_user_preferences_with_interactions(self):
        ...
        interactions = [
            {"query": "a very long detailed request about something specific" * 5, "rating": 5},
    ...
    def test_get_relevant_context_for_planning_meeting_goal(self):
        ...
        self.context_analyzer._infer_availability = MagicMock(return_value=[{"start": "2025-06-26T09:00:00", "end": "2025-06-26T10:00:00"}])
        context = self.context_analyzer.get_relevant_context_for_planning("user_1", "Schedule a meeting")
    ...
    def test_get_relevant_context_for_planning_research_goal(self):
        ...
        context = self.context_analyzer.get_relevant_context_for_planning("user_1", "Research new technology")
        ...
        self.assertEqual(context["goal_specific"]["constraints"]["focus_hours"], [9, 10, 11])
    ...
    def test_infer_availability_weekend(self):
        ...
        # Mock datetime to control the current time
        with patch('datetime.datetime') as mocked_datetime:
            mocked_datetime.now.return_value = datetime(2025, 6, 29, 12, 0, 0)  # Assuming Saturday
            availability = self.context_analyzer._infer_availability("user_1")
            self.assertTrue(len(availability) > 0)
            self.assertTrue(len(availability) <= 5)  # Limited to 5 slots
