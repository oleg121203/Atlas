...
class TestTaskPlannerAgent(unittest.TestCase):
    def setUp(self):
        ...
        self.self_learning_agent.get_user_learning_profile = MagicMock(return_value={
    ...
    def test_create_task_plan(self):
        ...
        self.assertEqual(plan["status"], "pending_user_consent")  # Updated due to ethical AI requirements
    ...
    def test_update_task_status(self):
        ...
        result = self.task_planner_agent.update_task_status(plan_id, task_id, "completed")
        self.assertTrue(result)
        updated_plan = self.task_planner_agent.active_plans[plan_id]
        updated_task = next((task for task in updated_plan["tasks"] if task["task_id"] == task_id), None)
    ...
    def test_update_task_status_without_consent(self):
        ...
        result = self.task_planner_agent.update_task_status(plan_id, task_id, "in_progress")
        self.assertFalse(result, "Task update should fail without consent")
        updated_plan = self.task_planner_agent.active_plans[plan_id]
        updated_task = next((t for t in updated_plan["tasks"] if t["task_id"] == task_id), None)
        self.assertIsNotNone(updated_task, "Task not found after update attempt")
        self.assertEqual(updated_task["status"], "pending", "Task status should remain pending without consent")
    ...
    def test_confirm_user_consent_and_update(self):
        ...
        updated_plan = self.task_planner_agent.active_plans[plan_id]
        self.assertEqual(updated_plan["status"], "active", "Plan status should be active after consent")
        self.assertEqual(updated_plan["ethical_flags"]["consent_status"], "granted", "Consent status should be granted")
        # Now try to update a task
        task_id = updated_plan["tasks"][0]["task_id"] if updated_plan["tasks"] else None
        self.assertIsNotNone(task_id, "No tasks found in plan")
        update_result = self.task_planner_agent.update_task_status(plan_id, task_id, "in_progress")
        ...
        final_task = next((t for t in final_plan["tasks"] if t["task_id"] == task_id), None)
        self.assertIsNotNone(final_task, "Task not found after update")
        self.assertEqual(final_task["status"], "in_progress", "Task status should be in_progress after update")
    ...
    def test_deny_user_consent(self):
        ...
        updated_plan = self.task_planner_agent.active_plans[plan_id]
        self.assertEqual(updated_plan["status"], "rejected_by_user", "Plan status should be rejected_by_user after denial")
        self.assertEqual(updated_plan["ethical_flags"]["consent_status"], "denied", "Consent status should be denied")
        # Try to update a task, should fail
        task_id = updated_plan["tasks"][0]["task_id"] if updated_plan["tasks"] else None
        self.assertIsNotNone(task_id, "No tasks found in plan")
        update_result = self.task_planner_agent.update_task_status(plan_id, task_id, "in_progress")
        self.assertFalse(update_result, "Task update should fail when consent is denied")
    ...
    def test_break_down_goal_meeting(self):
        ...
        self.assertTrue(len(tasks) == 1)  # Due to consolidation with high avg_rating, should be 1
        for i, task in enumerate(tasks):
            ...
            if i > 0:
                self.assertTrue(len(task["dependencies"]) >= 0)  # May or may not have dependencies after consolidation
