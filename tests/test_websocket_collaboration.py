...
class TestWebSocketCollaboration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = WebSocketClient(f"ws://localhost:8765/team/{self.team_id}", self.team_id, message_callback)
    ...
    def test_message_broadcast(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise
                time.sleep(2.0)  # Increased wait before retrying
        self.assertTrue(self.client.is_connected(), "First client failed to connect after multiple attempts")
        
        received_messages_second = []
        second_client = WebSocketClient(f"ws://localhost:8765/team/{self.team_id}", self.team_id, lambda msg: received_messages_second.append(msg))
        for attempt in range(max_retries):
            try:
                second_client.start()
                ...
                if attempt == max_retries - 1:
                    raise
                time.sleep(2.0)  # Increased wait before retrying
        self.assertTrue(second_client.is_connected(), "Second client failed to connect after multiple attempts")
        
        # Simulate sending a message through the server
        test_message = {"type": "team_message", "team_id": self.team_id, "content": "Test broadcast message"}
        try:
        ...
        self.assertGreaterEqual(len(self.received_messages), 1, "No broadcast message received by first client")
        found_test_message = False
        for msg in self.received_messages:
            if isinstance(msg, dict) and msg.get("content") == "Test broadcast message":
                found_test_message = True
                break
        self.assertTrue(found_test_message, "Test broadcast message content not received by first client")
        self.assertGreaterEqual(len(received_messages_second), 1, "No broadcast message received by second client")
        found_test_message = False
        for msg in received_messages_second:
            if isinstance(msg, dict) and msg.get("content") == "Test broadcast message":
                found_test_message = True
                break
        self.assertTrue(found_test_message, "Test broadcast message content not received by second client")
        second_client.stop()
