    ...
    def start_auto_reporting(self) -> bool:
        ...
        try:
            ...
            self.report_thread = threading.Thread(
                target=self._generate_reports_periodically
            )
            self.report_thread.daemon = True
            self.report_thread.start()
            logger.info(
                "Started auto-reporting for latency performance every 30 minutes."
            )
            return True
        except Exception as e:
        ...
        try:
            ...
            if self.report_thread:
                ...
                if self.report_thread.is_alive():
                    logger.warning(
                        "Auto-reporting thread did not terminate gracefully."
                    )
                else:
        ...
        try:
            if operation_name:
                if operation_name in self.latency_data:
                    latencies = [
                        entry["latency_ms"]
                        for entry in self.latency_data[operation_name]
                    ]
                    if latencies:
                        return {
                ...
                return {"operation": operation_name, "error": "Operation not found"}
            else:
                stats = {}
