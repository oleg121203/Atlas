...
def run_demo():
    ...
    manager.register_resource("cpu_cluster_1", ResourceType.CPU, 16.0, "cores", "dc1", 0.15)
    manager.register_resource("mem_bank_1", ResourceType.MEMORY, 64.0, "GB", "dc1", 0.05)
    manager.register_resource("storage_array_1", ResourceType.STORAGE, 1000.0, "GB", "dc1", 0.02)
    manager.register_resource("gpu_cluster_1", ResourceType.GPU, 4.0, "units", "dc1", 0.50)
    ...
    sched_1 = manager.schedule_workflow(workflow_id, start_time, 2.0, required_resources, PriorityLevel.HIGH)
    ...
    sched_2 = manager.schedule_workflow(workflow_id_2, start_time_2, 1.5, required_resources_2, PriorityLevel.MEDIUM)
    ...
    budget = 0.30
    opt_result = manager.optimize_costs(workflow_id, cloud_resources, budget, performance_reqs)
    ...
    for alloc_id in allocations.keys():
