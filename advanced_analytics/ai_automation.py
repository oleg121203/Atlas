...
class AIAutomation:
        ...
        self.task_data = pd.DataFrame(
            {
                ...
                "due_time": [
                    date_range[i] + pd.Timedelta(hours=np.random.randint(1, 48))
    ...
    def prioritize_tasks(self, tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            n_clusters = min(
                len(tasks), 3
            )  # Limit clusters to 3 or number of tasks if fewer
            if n_clusters < 2:
            ...
            for i in range(n_clusters):
                cluster_points = [
                    feature_matrix[j][0] for j in range(len(tasks)) if clusters[j] == i
                ]
                urgency = (
                    sum(cluster_points) / len(cluster_points) if cluster_points else 0
                )
            ...
            cluster_priority = {
                cluster_id: rank
                for rank, (cluster_id, _) in enumerate(
                    sorted(centroid_urgency, key=lambda x: x[1], reverse=True)
                )
            }
            ...
            return sorted(
                tasks, key=lambda x: ["high", "medium", "low"].index(x["priority"])
            )
                ...
                if len(parts) > 1:
                    ...
                    for due_indicator in due_indicators:
                        if due_indicator in task_part:
                            ...
                            if len(due_split) > 1 and due_indicator not in [
                                "today",
                                "tomorrow",
                            ]:
                                ...
                                if start_index_due != -1:
                                    due_date = text[
                                        start_index_due : start_index_due
                                        + len(due_date_lower)
                                    ]
                                else:
    ...
    def recommend_workflow(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            completion_rate = (
                user_tasks["completed"].mean()
                if "completed" in user_tasks.columns
                else 0
            )
            ...
            if (
                "duration_hours" in user_tasks.columns
                and "estimated_duration_hours" in user_tasks.columns
            ):
                avg_duration_diff = (
                    user_tasks["duration_hours"]
                    - user_tasks["estimated_duration_hours"]
                ).mean()
                if (
                    avg_duration_diff > 1.0
                ):  # Tasks take significantly longer than estimated
                    recommendations.append(
                        {
            ...
            if (
                len(user_tasks["name"].str.lower().value_counts())
                < len(user_tasks) * 0.5
            ):  # Many repeated task names
                recommendations.append(
                    {
