...
from atlas.ui.input_validation import sanitize_form_data, validate_form_data
...
class SettingsWidget(QWidget):
    ...
    def save_settings(self) -> None:
        ...
        form_data = {"API Key": (self.api_input.text(), "password")}
        ...
        logger.debug(
            "API key sanitized, original length: %d, sanitized length: %d",
            len(self.api_input.text()),
            len(sanitized_api_key),
        )
        ...
        logger.info(
            "Settings saved - Theme: %s, Language: %s, API Key: [REDACTED]",
            theme,
            language,
        )
