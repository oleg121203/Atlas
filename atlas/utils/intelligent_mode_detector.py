...
class IntelligentModeDetector:
    ...
    def detect_chat_mode(
        ...
        simple_score, simple_reasoning = self._check_simple_commands(
            message, message_lower
        )

        # Фаза 2: Verification складних запитів
        advanced_score, advanced_reasoning = self._check_advanced_thinking(
            message, message_lower
        )
    ...
    def _check_simple_commands(
        ...
        if len(message.split()) <= 4 and any(
            cmd in message_lower for cmd in ["read", "show", "list", "tree"]
        ):
            score += 0.3
        ...
        reasoning = (
            f"Simple command indicators: {', '.join(matched_patterns)}"
            if matched_patterns
            else "No simple patterns"
        )
        return min(score, 1.0), reasoning

    def _check_advanced_thinking(
        self, message: str, message_lower: str
    ) -> Tuple[float, str]:
        ...
        reasoning = (
            f"Advanced thinking indicators: {', '.join(matched_patterns)}"
            if matched_patterns
            else "No advanced patterns"
        )
        return min(score, 1.0), reasoning
    ...
    def _analyze_context_modifiers(self, message: str, message_lower: str) -> float:
        ...
        complexity_indicators = self.patterns["context_modifiers"][
            "complexity_indicators"
        ]
        ...
        # Індикатори простоти
        simplicity_indicators = self.patterns["context_modifiers"][
            "simplicity_indicators"
        ]
        simplicity_count = sum(
            1 for indicator in simplicity_indicators if indicator in message_lower
        )
    ...
    def _resolve_mode_conflict(
        self,
        simple_score: float,
        advanced_score: float,
        context_modifier: float,
        simple_reasoning: str,
        advanced_reasoning: str,
        ...
        if simple_score >= 0.8 and adjusted_advanced_score < 0.4:
        ...
        elif adjusted_advanced_score >= 0.4 and (
            adjusted_advanced_score > simple_score or simple_score < 0.3
        ):
            ...
            has_complex_terms = any(
                term in original_message.lower()
                for term in [
                    "проаналізуй",
                    "архітектур",
                    "що не так",
    ...
    def _log_detection(self, message: str, result: DetectionResult):
        ...
        self.logger.debug(
            f"Mode detection: '{message[:50]}...' -> {result.mode.value} (confidence: {result.confidence:.2f})"
        )
    ...
    def add_feedback(
        self,
        message: str,
        detected_mode: ChatMode,
        actual_mode: ChatMode,
...
def test_mode_detector():
    ...
    for message, expected_mode in test_cases:
        ...
        is_correct = result.mode == expected_mode or (
            result.mode == ChatMode.HYBRID
            and result.should_use_advanced
            == (expected_mode == ChatMode.ADVANCED_THINKING)
        )
        ...
        print(
            f"    Детектовано: {result.mode.value} (впевненість: {result.confidence:.2f})"
        )
