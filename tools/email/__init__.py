...
class EmailTool:
    ...
    def search_emails(self, 
        ...
        try:
            ...
            if categories:
                search_query += " " + " OR ".join(f"category:{cat}" for cat in categories)
            
            # Add importance filter
            if importance:
                search_query += f" importance:{importance}"
            ...
            for msg in messages:
                ...
                # Analyze message
                analysis = self.analytics.analyze_email(message)
                ...
                detailed_messages.append({
                    ...
                    'analysis': analysis,
                    'attachments': [p for p in message['payload'].get('parts', []) 
                                  if p.get('filename')],
                    'labels': message.get('labelIds', [])
                })
            
            # Filter by thread length
            if thread_length:
                detailed_messages = [m for m in detailed_messages 
                                  if len(m['analysis'].get('thread', [])) 
                                  >= thread_length[0] 
                                  and len(m['analysis'].get('thread', [])) 
                                  <= thread_length[1]]
            
            # Filter by response time
            if response_time:
                detailed_messages = [m for m in detailed_messages 
                                  if m['analysis'].get('response_time', 0) 
                                  >= response_time[0] 
                                  and m['analysis'].get('response_time', 0) 
                                  <= response_time[1]]
