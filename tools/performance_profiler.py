...
class PerformanceProfiler:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        if re.search(pattern, line):
                            ...
                            issue = PerformanceIssue(
                                ...
                                impact_estimate=self._estimate_impact(category, in_loop),
    ...
    def _is_in_loop_context(self, lines: List[str], line_num: int) -> bool:
        ...
        for i in range(max(0, line_num - 20), line_num):
            ...
            if line_indent < indent_level and any(keyword in line for keyword in loop_keywords):
                return True
    ...
    def _estimate_impact(self, category: str, in_loop: bool) -> str:
        ...
        if in_loop:
            base_impact = "Critical - " + base_impact + " (multiplied by loop iterations)"
        
        return base_impact
    
    def _analyze_ast_performance(self, file_path: Path, content: str) -> List[PerformanceIssue]:
        """Analyze AST for complex performance patterns."""
        issues = []
        
        try:
            tree = ast.parse(content)
            analyzer = PerformanceASTAnalyzer(str(file_path.relative_to(self.root_path)))
            analyzer.visit(tree)
    ...
    def _analyze_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_analysis = {
                ...
                'total_traced_memory_mb': sum(stat.size for stat in top_stats) / (1024**2),
    ...
    def _generate_performance_recommendations(self, issues: List[PerformanceIssue], 
        ...
        if critical_issues:
            recommendations.append(f"🚨 **URGENT**: Fix {len(critical_issues)} critical performance issues immediately")
        ...
        if issue_categories['blocking_calls'] > 0:
            recommendations.append("🔄 **Async Optimization**: Convert blocking calls to async operations")
        
        if issue_categories['inefficient_loops'] > 0:
            recommendations.append("🔁 **Loop Optimization**: Optimize loop patterns for better performance")
    ...
    def _create_performance_summary(self, issues: List[PerformanceIssue], 
                                  system_metrics: Dict[str, Any], 
                                  memory_usage: Dict[str, Any]) -> str:
    ...
    def generate_performance_report(self) -> str:
        ...
        if issue_categories:
            ...
            for category, issues in sorted(issue_categories.items(), key=lambda x: len(x[1]), reverse=True):
                ...
                for file_path, count in sorted(file_counts.items(), key=lambda x: x[1], reverse=True)[:3]:
        ...
        if analysis.system_metrics:
            ...
            report.append(f"- **CPU**: {cpu.get('usage_percent', 'N/A')}% usage, {cpu.get('count', 'N/A')} cores")
            report.append(f"- **Memory**: {memory.get('used_percent', 'N/A')}% used ({memory.get('available_gb', 'N/A'):.1f}GB available)")
