...
SENSITIVE_PATTERNS = [
    re.compile(r"(password|secret|key|token|api_key|access_token)=[^&\s]+", re.IGNORECASE),
...
def setup_logging() -> logging.Logger:
    ...
    file_formatter = RedactingFormatter("%(asctime)s - %(name)s - %(levelname)s - %(pathname)s:%(lineno)d - %(message)s")
