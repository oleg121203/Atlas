...
try:
    ...
    def load_config(
        config_path: Optional[str] = None, environment: str = "dev"
    ) -> Dict[str, Any]:
...
_performance_metrics = {"inference_times": {}, "success_rates": {}, "error_counts": {}}
...
class AIModelManager:
    ...
    def __init__(self, config_path: Optional[str] = None, environment: str = "dev"):
        ...
        if not hasattr(self, "_initialized") or not self._initialized:
            ...
            self.storage_path = Path(
                self.config.get("ai_models_storage", "config/ai_models.json")
            )
            ...
            logger.info(
                "AIModelManager initialized for environment: %s", self.environment
            )
    ...
    def load_models(self) -> None:
        ...
        try:
            ...
            env_overrides = self.config.get("ai_model_overrides", {}).get(
                self.environment, {}
            )
            if env_overrides:
                self.models.update(env_overrides)
                logger.info(
                    "Applied environment-specific overrides for: %s", self.environment
                )
    ...
    def save_models(self) -> None:
        ...
        try:
            ...
            logger.error(
                "Error saving AI model configurations: %s", str(e), exc_info=True
            )
            raise AIIntegrationError(
                f"Failed to save AI model configurations: {str(e)}"
            ) from e
    ...
    def infer(
        self, model_name: str, input_data: Any, context: Optional[Dict] = None
    ) -> Any:
        """
    ...
    def _infer_openai(
        self, model_config: Dict, input_data: Any, context: Optional[Dict] = None
    ) -> Any:
        """
        ...
        try:
            ...
            raise AIIntegrationError(f"OpenAI inference failed: {str(e)}") from e
    ...
    def _infer_anthropic(
        ...
        try:
            ...
            raise AIIntegrationError(f"Anthropic inference failed: {str(e)}") from e
    ...
    def _infer_local(
        ...
        try:
            ...
            return (
                "Local AI model inference is not implemented in this version of Atlas."
            )
        except Exception as e:
            logger.error("Error in local model inference: %s", str(e), exc_info=True)
            raise AIIntegrationError(f"Local model inference failed: {str(e)}") from e

    def get_suggestion(
        self, model_name: str, context: Dict, prompt_type: str = "general"
    ) -> str:
        """
        ...
        """
        logger.info(
            "Generating suggestion with model: %s, type: %s", model_name, prompt_type
        )
        model_config = self.get_model(model_name)
        if not model_config:
            raise AIIntegrationError(f"Model {model_name} not found")
...
Please provide a clear and concise response that directly addresses my request."""
    ...
    def automate_task(
        self, model_name: str, task_description: str, context: Optional[Dict] = None
    ) -> Dict:
        """
...
{json.dumps(context, indent=2)}"""
        ...
        try:
            ...
            logger.warning(
                "Could not parse automation response as JSON, returning raw text"
            )
...
def _record_performance(
    ...
    if not success:
        ...
        _performance_metrics["error_counts"][model_name][error_type] = (
            _performance_metrics["error_counts"][model_name].get(error_type, 0) + 1
        )

    logger.debug(
        f"Performance recorded for {model_name}: Duration={duration:.2f}s, Success={success}"
    )
...
def get_performance_report(model_name: str = None) -> Dict:
    ...
    report = {}
    models = (
        [model_name] if model_name else _performance_metrics["inference_times"].keys()
    )

    for model in models:
        if model in _performance_metrics["inference_times"]:
            times = _performance_metrics["inference_times"][model]
            success_data = _performance_metrics["success_rates"].get(
...
def optimize_model_selection(context: Dict) -> str:
    ...
    if performance_report:
        ...
        if best_model and best_model != preferred_model:
            logger.info(
                f"Optimized model selection: Using {best_model} instead of {preferred_model} based on performance"
            )
            return best_model
...
def get_ai_suggestion(
    ...
    """
    return get_ai_model_manager(config_path, environment).get_suggestion(
        model_name, context, prompt_type
    )


def automate_ai_task(
    model_name: str,
    ...
    """
    return get_ai_model_manager(config_path, environment).automate_task(
        model_name, task_description, context
    )
