...
class NetworkClient:
    ...
    def get(
        self, url: str, timeout: int = 10, validate_ssl: bool = True, **kwargs
        ...
        if validate_ssl:
            ...
            if not is_valid:
                logger.error(
                    "SSL validation failed for GET request to %s: %s", url, message
                )
                return None
    ...
    def post(
        ...
        **kwargs,
        ...
        if validate_ssl:
            ...
            if not is_valid:
                logger.error(
                    "SSL validation failed for POST request to %s: %s", url, message
                )
                return None

        return make_secure_request(
            url, method="POST", data=data, json=json, timeout=timeout, **kwargs
        )
    ...
    def request(
        ...
        **kwargs,
        ...
        if validate_ssl:
            ...
            if not is_valid:
                logger.error(
                    "SSL validation failed for %s request to %s: %s",
                    method,
                    url,
                    message,
                )
