...
class WorkflowManager:
    ...
    def create_workflow(self, workflow_id: str, definition: Dict) -> bool:
        ...
        try:
            if workflow_id in self.workflows:
                ...
                raise_alert("warning", f"Workflow Overwrite: {workflow_id}", "Existing workflow overwritten with new definition")
    ...
    def start_workflow(self, workflow_id: str) -> bool:
        ...
        try:
            if workflow_id not in self.workflows:
                ...
                raise_alert("error", f"Workflow Not Found: {workflow_id}", "Cannot start non-existent workflow")
                return False
    ...
    def execute_step(self, workflow_id: str, step_id: Optional[str] = None) -> bool:
        ...
        try:
            if workflow_id not in self.workflows:
                logger.error(f"Workflow {workflow_id} not found")
                raise_alert("error", f"Workflow Not Found: {workflow_id}", "Cannot execute step in non-existent workflow")
                return False
            ...
            if not step_definition:
                ...
                raise_alert("error", f"Step Not Found: {current_step}", f"Step not defined in workflow {workflow_id}")
                return False
            ...
            if result["success"]:
                workflow["state"]["current_step"] = step_definition.get("next_step", "end")
                workflow["state"]["status"] = "running" if workflow["state"]["current_step"] != "end" else "completed"
                ...
                raise_alert("error", f"Step Execution Failed: {current_step}", result["error"])
            ...
            logger.info(f"Step {current_step} execution {'succeeded' if result['success'] else 'failed'} in workflow {workflow_id}")
            self._persist_workflow_state(workflow_id)
            return result["success"]
        except Exception as e:
            logger.error(f"Error executing step {step_id or 'current'} in workflow {workflow_id}: {str(e)}")
    ...
    def retry_step(self, workflow_id: str) -> bool:
        ...
        try:
            if workflow_id not in self.workflows:
                logger.error(f"Workflow {workflow_id} not found")
                raise_alert("error", f"Workflow Not Found: {workflow_id}", "Cannot retry non-existent workflow")
                return False
            ...
            if workflow["state"]["retry_count"] >= max_retries:
                ...
                raise_alert("error", f"Max Retries Reached: {workflow_id}", f"Workflow has reached maximum retry limit of {max_retries}")
                workflow["state"]["status"] = "failed"
                self._persist_workflow_state(workflow_id)
                return False
            
            logger.info(f"Retrying step {workflow['state']['current_step']} in workflow {workflow_id}")
    ...
    def rollback_workflow(self, workflow_id: str) -> bool:
        ...
        try:
            if workflow_id not in self.workflows:
                logger.error(f"Workflow {workflow_id} not found")
                raise_alert("error", f"Workflow Not Found: {workflow_id}", "Cannot rollback non-existent workflow")
                return False
            ...
            if last_successful:
                ...
                workflow["state"]["retry_count"] = 0
                workflow["state"]["last_updated"] = datetime.now().isoformat()
                logger.info(f"Workflow {workflow_id} rolled back to step {last_successful}")
                raise_alert("info", f"Workflow Rolled Back: {workflow_id}", f"Rolled back to last successful step: {last_successful}")
                self._persist_workflow_state(workflow_id)
                return True
            else:
                logger.warning(f"No successful step found to rollback workflow {workflow_id}")
                raise_alert("warning", f"Rollback Failed: {workflow_id}", "No successful step found in history to rollback to")
                workflow["state"]["status"] = "failed"
                self._persist_workflow_state(workflow_id)
                return False
        except Exception as e:
    ...
    def _persist_workflow_state(self, workflow_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to persist workflow state for {workflow_id}: {str(e)}")
