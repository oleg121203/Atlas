...
import security.security_utils as security_utils
...
DEFAULT_ROLE_PERMISSIONS: Dict[Role, Set[Permission]] = {
    Role.ADMIN: {
        # Full system access
        Permission.SYSTEM_CONFIG_READ, Permission.SYSTEM_CONFIG_WRITE,
        ...
        Permission.PLUGIN_INSTALL, Permission.PLUGIN_UNINSTALL,
        Permission.PLUGIN_ENABLE, Permission.PLUGIN_DISABLE,
        Permission.PLUGIN_CONFIG_READ, Permission.PLUGIN_CONFIG_WRITE,
        # Full module data access
        Permission.MODULE_DATA_READ, Permission.MODULE_DATA_WRITE,
        # Full agent control
        Permission.AGENT_CREATE, Permission.AGENT_CONTROL, Permission.AGENT_DELETE,
        # Full task management
        Permission.TASK_CREATE, Permission.TASK_READ, 
        Permission.TASK_UPDATE, Permission.TASK_DELETE,
        # Full chat access
        Permission.CHAT_SEND, Permission.CHAT_READ, Permission.CHAT_DELETE
    },
    Role.MANAGER: {
    ...
    Role.USER: {
        ...
        # Limited chat access
        Permission.CHAT_SEND, Permission.CHAT_READ
    },
    Role.GUEST: {
        # No system access
        # No user management
...
class RBACManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        self.role_permissions: Dict[Role, Set[Permission]] = DEFAULT_ROLE_PERMISSIONS.copy()
    ...
    def load_config(self) -> None:
        ...
        if not os.path.exists(self.config_path):
            logger.info("No RBAC config file found at %s, using default permissions", self.config_path)
            return
        ...
        try:
            ...
            for role_str, perms in custom_permissions.items():
                try:
                    role = Role(role_str)
                    self.role_permissions[role] = {Permission(p) for p in perms if p in Permission.__members__.values()}
    ...
    def check_permission(self, username: str, permission: Permission) -> bool:
        ...
        if role is None:
            logger.warning("No role assigned to user %s, denying permission %s", username, permission.value)
            return False
        
        has_permission = permission in self.role_permissions[role]
        if not has_permission:
            logger.warning("Permission %s denied for user %s with role %s", 
                          permission.value, username, role.value)
    ...
    def get_user_permissions(self, username: str) -> Set[Permission]:
        ...
        role = self.get_user_role(username)
        if role is None:
            logger.warning("No role assigned to user %s, no permissions available", username)
            return set()
    ...
    def remove_role_permission(self, role: Role, permission: Permission) -> None:
        ...
        if permission in self.role_permissions[role]:
            ...
            logger.warning("Permission %s not found for role %s", permission.value, role.value)
    
    def enforce_permission(self, username: str, permission: Permission, 
                          operation_description: str = "Operation") -> None:
        """
        ...
        if not self.check_permission(username, permission):
            role = self.get_user_role(username)
            error_msg = (f"{operation_description} not allowed for user {username} "
                        f"with role {role.value if role else 'None'}. "
                        f"Required permission: {permission.value}")
            logger.error(error_msg)
            raise PermissionError(error_msg)
        logger.debug("Permission %s granted for user %s for %s", 
                    permission.value, username, operation_description)
