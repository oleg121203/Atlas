...
class MemoryPanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # Список колекцій
        ctk.CTkLabel(self, text="Collections", font=ctk.CTkFont(weight="bold")).grid(
        ...
        ctk.CTkButton(self, text="Search", command=self._search).grid(
        ...
        ctk.CTkButton(btn_frame, text="Clear", command=self._clear).pack(
            side="left", padx=5
        )
