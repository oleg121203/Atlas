...
class FallbackChainEditor(ctk.CTkToplevel):
    ...
    def __init__(
        ...
        all_models: List[str],
        save_callback: Callable[[List[str]], None],
    ):
    ...
    def _create_widgets(self):
        ...
        ctk.CTkButton(add_frame, text="Add", width=50, command=self._add_model).grid(
        ...
        ctk.CTkButton(reorder_frame, text="Remove", command=self._remove_model).pack(
            pady=20
        )
        ...
        ctk.CTkButton(button_frame, text="Save", command=self._save_and_close).pack(
            side="left", padx=5
        )
        ctk.CTkButton(button_frame, text="Cancel", command=self.destroy).pack(
            side="left"
        )
