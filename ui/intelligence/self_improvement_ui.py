...
class SelfImprovementUI(QWidget):
    ...
    def connect_to_engine(self):
        ...
        if self.improvement_engine:
            self.improvement_engine.improvement_identified.connect(self.on_improvement_identified)
            self.improvement_engine.improvement_plan_updated.connect(self.on_plan_updated)
            self.improvement_engine.improvement_executed.connect(self.on_improvement_executed)
    ...
    def update_improvement_display(self) -> None:
        ...
        if self.improvement_engine:
            ...
            for area_type, area_data in self.improvement_engine.improvement_areas.items():
                area_item = QTreeWidgetItem(self.improvement_tree, [area_type, ""])
                QTreeWidgetItem(area_item, ["Issue", area_data.get("issue", "N/A")])
                QTreeWidgetItem(area_item, ["Current Value", str(area_data.get("current_value", "N/A"))])
                QTreeWidgetItem(area_item, ["Target Value", str(area_data.get("target_value", "N/A"))])

            # Display improvement plans
            for area_type, plan in self.improvement_engine.improvement_plans.items():
                plan_item = QTreeWidgetItem(self.improvement_tree, [f"Plan: {area_type}", ""])
                QTreeWidgetItem(plan_item, ["Expected Outcome", plan.get("expected_outcome", "N/A")])
            ...
            if history:
                history_item = QTreeWidgetItem(self.improvement_tree, ["Improvement History", ""])
                for i, result in enumerate(history[:5]):  # Limit to last 5 improvements
                    result_text = f"Improvement {i+1}: {result.get('area', 'Unknown')}"
                    result_item = QTreeWidgetItem(history_item, [result_text, ""])
                    QTreeWidgetItem(result_item, ["Success", str(result.get('success', 'N/A'))])
                    QTreeWidgetItem(result_item, ["Message", result.get('message', 'N/A')])
                    QTreeWidgetItem(result_item, ["Strategy", result.get('strategy_used', 'N/A')])

        else:
            placeholder_item = QTreeWidgetItem(self.improvement_tree, ["Placeholder", ""])
