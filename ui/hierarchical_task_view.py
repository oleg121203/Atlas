...
class HierarchicalTaskView(ctk.CTkFrame):
    ...
    def __init__(
        self, master, task_callback: Optional[Callable[[str, str, Any], None]] = None
    ):
        """
    ...
    def _create_widgets(self):
        ...
        tree_header = ctk.CTkLabel(
            left_panel,
            text="Hierarchical Task Structure",
        ...
        self.progress_label = ctk.CTkLabel(
            left_panel, text="", font=ctk.CTkFont(size=10), text_color="gray"
        )
        ...
        self.task_tree = ctk.CTkScrollableFrame(
            left_panel, label_text="Tasks", height=400
        )
        ...
        details_header = ctk.CTkLabel(
            right_panel, text="Task Details", font=ctk.CTkFont(size=16, weight="bold")
        )
        details_header.grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")

        # Task details frame
        self.task_details_frame = ctk.CTkScrollableFrame(
            right_panel, label_text="Information", height=300
        )
        self.task_details_frame.grid(
            row=1, column=0, sticky="nsew", padx=10, pady=(5, 10)
        )

        # Task controls frame
        self.task_controls_frame = ctk.CTkFrame(right_panel)
        self.task_controls_frame.grid(
    ...
    def _add_task_frame_to_ui(self, task_frame_data: Dict[str, Any]):
        ...
        try:
            ...
            status_label = ctk.CTkLabel(
                task_frame, text="●", text_color=status_color, font=ctk.CTkFont(size=16)
            )
            ...
            title_label = ctk.CTkLabel(
                task_frame, text=display_text, font=ctk.CTkFont(size=11, weight="bold")
            )
            ...
            progress_label = ctk.CTkLabel(
                task_frame, text=progress_text, font=ctk.CTkFont(size=10)
            )
            ...
            level_label = ctk.CTkLabel(
                task_frame, text=level_emoji, font=ctk.CTkFont(size=12)
            )
            level_label.grid(row=0, column=4, padx=5, pady=5)

            # Bind click event
            task_frame.bind(
                "<Button-1>", lambda e, tid=task_id: self._on_task_click(tid)
            )
            title_label.bind(
                "<Button-1>", lambda e, tid=task_id: self._on_task_click(tid)
            )
        ...
        def async_update():
            try:
                ...
                self.update_queue.put(
                    {"type": "progress", "text": "Building task tree..."}
                )
                ...
                self.after(
                    2000,
                    lambda: self.update_queue.put({"type": "progress", "text": ""}),
                )

            except Exception as e:
                self.logger.error(f"Error in async plan update: {e}")
                self.update_queue.put({"type": "progress", "text": f"Error: {str(e)}"})
    ...
    def _build_task_tree_async(self, task_id: str, level: int):
        ...
        self.update_queue.put(
            {
                "type": "task_frame",
                "data": {"task_id": task_id, "level": level, "task": task},
            }
        )
    ...
    def _update_task_in_tree(self, task_id: str):
        ...
        if len(task_frame.winfo_children()) > 2:
            ...
            if isinstance(progress_bar, ctk.CTkProgressBar) and isinstance(
                progress_label, ctk.CTkLabel
            ):
                progress = task.get("progress", 0.0)
                progress_bar.set(progress)
    ...
    def _show_task_details(self, task: Dict[str, Any]):
        ...
        title_label = ctk.CTkLabel(
            ...
            text=task.get("title", "Unknown Task"),
            font=ctk.CTkFont(size=16, weight="bold"),
        )
        ...
        desc_label = ctk.CTkLabel(
            self.task_details_frame, text=description, wraplength=300, justify="left"
        )
        ...
        level_label = ctk.CTkLabel(
            metadata_frame, text=f"Level: {level_emoji} {level.title()}"
        )
        ...
        if created_at:
            ...
            created_label = ctk.CTkLabel(
                timestamps_frame, text=f"Created: {created_time}"
            )
        ...
        if started_at:
            ...
            started_label = ctk.CTkLabel(
                timestamps_frame, text=f"Started: {started_time}"
            )
        ...
        if completed_at:
            ...
            completed_label = ctk.CTkLabel(
                timestamps_frame, text=f"Completed: {completed_time}"
            )
    ...
    def _get_level_emoji(self, level: str) -> str:
        ...
        emojis = {"strategic": "🎯", "tactical": "📋", "operational": "⚡"}
    ...
    def handle_websocket_task_update(self, task_data: Dict[str, Any]):
        ...
        try:
            self.logger.info(
                f"Received WebSocket task update: {task_data.get('id', 'unknown')}"
            )
            self.update_task(task_data)
        except Exception as e:
            self.logger.error(
                f"Error handling WebSocket task update: {e}", exc_info=True
            )
