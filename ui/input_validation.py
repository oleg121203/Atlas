...
# Input field types
INPUT_TYPES = {
    "email": {"max_length": 100, "validator": lambda x: validate_input(x, "email")},
    "url": {"max_length": 200, "validator": lambda x: validate_input(x, "url")},
    "filepath": {
        "max_length": 255,
    ...
    "text": {"max_length": 1000, "validator": lambda x: validate_input(x, "text")},
    "alphanumeric": {
        "max_length": 100,
...
def validate_ui_input(
    value: str, input_type: str, field_name: str = "Input"
) -> tuple[bool, str]:
    """
...
def sanitize_ui_input(value: str) -> str:
    ...
    logger.debug(
        "Input sanitized, original length: %d, sanitized length: %d",
        len(value),
        len(sanitized),
    )
    return sanitized
