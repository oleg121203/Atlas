...
class UserManagementWidget(QWidget):
    ...
    def refresh_user_list(self) -> None:
        ...
        for username in self.rbac_manager.user_roles:
    ...
    def add_or_update_user(self) -> None:
        ...
        logger.debug(
            "Username sanitized, original: %s, sanitized: %s",
            username,
            sanitized_username,
        )
        ...
        QMessageBox.information(
            self,
            "Success",
            f"User {sanitized_username} added/updated with role {role_str}",
        )
        ...
        if (
            self.user_list.currentItem()
            and self.user_list.currentItem().text() == sanitized_username
        ):
