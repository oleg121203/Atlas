...
class SystemControlPanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            self, text="System Control", font=ctk.CTkFont(size=16, weight="bold")
        ).grid(row=0, column=0, columnspan=3, pady=(10, 5))

        # Help button
        ctk.CTkButton(
            self, text="?", width=30, command=self._show_help, fg_color="#3A7CA5"
        ).grid(row=0, column=2, padx=10, pady=(10, 5), sticky="e")
        ...
        # Open App
        ctk.CTkLabel(self, text="Open App:").grid(
        ...
        # Set Volume
        ctk.CTkLabel(self, text="Set Volume:").grid(
        ...
        refresh_vol_btn = ctk.CTkButton(
            self, text="Refresh Volume", command=self._refresh_volume
        )
        refresh_vol_btn.grid(
        ...
        self.apps_label.grid(
            row=9, column=0, columnspan=2, padx=10, pady=(10, 0), sticky="w"
        )
        refresh_apps_btn = ctk.CTkButton(
            self, text="Refresh Apps", command=self._refresh_apps
        )
        refresh_apps_btn.grid(
        ...
        self.feedback_label.grid(
            row=11, column=0, columnspan=2, padx=10, pady=(10, 0), sticky="w"
        )

        # Status bar
        self.status_bar = ctk.CTkLabel(
            self, text="Ready", anchor="w", fg_color="#222", text_color="#fff"
        )
        self.status_bar.grid(
            row=12, column=0, columnspan=3, sticky="ew", padx=0, pady=(10, 0)
        )
    ...
    def _show_help(self):
        ...
        ctk.CTkLabel(
            dialog,
            text="System Control Panel Help",
        ...
        ctk.CTkLabel(dialog, text=help_text, wraplength=360, justify="left").pack(
            padx=20, pady=10
        )
    ...
    def _show_feedback(self, result):
        if result.get("status") == "success":
            ...
            self.feedback_label.configure(
                text=f"Error: {result.get('error')}", text_color="#B22222"
            )
