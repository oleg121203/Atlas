...
class SettingsPanel(ctk.CTkFrame):
    def __init__(self, master, openai_api_key_var, gemini_api_key_var, groq_api_key_var, mistral_api_key_var, anthropic_api_key_var,
                 plugin_manager, plugin_enabled_vars, loaded_settings, on_save_settings, on_reset_tokens, on_toggle_all_plugins, on_populate_plugins, **kwargs):
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(plugin_buttons_frame, text="Enable All", width=90, command=lambda: self.on_toggle_all_plugins(True)).pack(side="left", padx=5)
        ctk.CTkButton(plugin_buttons_frame, text="Disable All", width=90, command=lambda: self.on_toggle_all_plugins(False)).pack(side="left")
        ...
        self.reset_tokens_button = ctk.CTkButton(token_frame, text="Reset", command=self.on_reset_tokens)
        self.reset_tokens_button.grid(row=1, column=2, rowspan=3, padx=10, pady=5)

        # Save Button
        self.save_settings_button = ctk.CTkButton(self, text="Save Settings", command=self.on_save_settings)
