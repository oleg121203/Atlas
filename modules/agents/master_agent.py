...
try:
    from modules.agents.tool_execution import InvalidToolArgumentsError, ToolNotFoundError  # type: ignore[import-untyped]
except ImportError:
...
class MasterAgent:
    ...
    def _initialize_agents(self) -> None:  # noqa: D401
        ...
        """
        try:
            from modules.agents.browser_agent import BrowserAgent  # local import to avoid hard dependency at module import time
            self._browser_agent = BrowserAgent()
        except Exception:  # pragma: no cover – optional dependency
            self._browser_agent = None

        try:
            from modules.agents.advanced_application_agent import AdvancedApplicationAgent  # noqa: WPS433 – local import by design
