...
IS_APPLE_SILICON = IS_MACOS and (
    platform.machine() == "arm64" or platform.machine().startswith("arm")
)
...
def get_screenshot_method():
    ...
    if IS_MACOS and importlib.util.find_spec("Quartz"):
        methods.append("quartz")

    # Check for PyAutoGUI availability
    if not IS_HEADLESS and importlib.util.find_spec("pyautogui"):
