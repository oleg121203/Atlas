...
class GeminiProvider:
    ...
    def _initialize_client(self):
        ...
        if not GEMINI_AVAILABLE:
            self.logger.warning("Gemini library not installed. Gemini client is not available.")
            return
    ...
    def chat(self, messages: List[Dict[str, Any]], tools: Optional[List[Dict[str, Any]]] = None, model: Optional[str] = None, max_tokens: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if max_tokens is not None:
                self.logger.warning("max_tokens parameter is not supported in Gemini API")

            response = chat_session.history[-1].parts[0].text if chat_session.history else ""
            return {
                "content": response,
                "tool_calls": None,
                "prompt_tokens": 0,
