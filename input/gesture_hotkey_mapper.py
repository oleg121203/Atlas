...
class GestureHotkeyMapper:
    ...
    def register_hotkey(self, hotkey: str, command: Dict[str, Any]) -> None:
        ...
        if self.is_mac and 'ctrl' in hotkey:
            ...
            print(f"Registered macOS hotkey '{mac_hotkey}' for action: {command.get('action', 'unknown')}")
            if keyboard and self.is_listening:
                try:
                    keyboard.add_hotkey(mac_hotkey, self._handle_hotkey, args=(mac_hotkey,))
                except (ValueError, OSError) as e:
                    print(f"Failed to register macOS hotkey '{mac_hotkey}': {e}")
                    print("Hotkey functionality may be limited on this system. On macOS, you may need to run the script with sudo or adjust permissions for keyboard access.")
        
        self.hotkeys[hotkey] = command
        print(f"Registered hotkey '{hotkey}' for action: {command.get('action', 'unknown')}")
        if keyboard and self.is_listening:
    ...
    def register_gesture(self, gesture_name: str, command: Dict[str, Any]) -> None:
        ...
        """
        print(f"Gesture support for '{gesture_name}' is not implemented on this platform")
        # On macOS, we skip gesture support due to complexity with trackpad gestures
    
    def map_workflow_commands(self, workflow_id: str, commands: Dict[str, Dict[str, Any]]) -> None:
