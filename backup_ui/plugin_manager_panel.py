...
class PluginManagerPanel(ctk.CTkFrame):
    def __init__(self, master, plugin_manager, plugin_enabled_vars, on_toggle_all_plugins, on_plugin_toggle, **kwargs):
    ...
    def _build_ui(self):
        ...
        # Header
        header = ctk.CTkLabel(self, text="Available Plugins", font=ctk.CTkFont(weight="bold"))
        header.grid(row=0, column=0, sticky="w", padx=10, pady=(10, 5))
        # Buttons
        btn_frame = ctk.CTkFrame(self)
        btn_frame.grid(row=0, column=1, sticky="e", padx=10, pady=(10, 5))
        ctk.CTkButton(btn_frame, text="Enable All", width=90, command=lambda: self.on_toggle_all_plugins(True)).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Disable All", width=90, command=lambda: self.on_toggle_all_plugins(False)).pack(side="left")
        # Scrollable list
    ...
    def _populate_plugins(self):
        ...
        if not all_plugins:
            ctk.CTkLabel(self.scroll_frame, text="No plugins found.", text_color="gray").pack(padx=10, pady=10)
        else:
            for plugin_id, plugin_data in sorted(all_plugins.items()):
                ...
                cb = ctk.CTkCheckBox(self.scroll_frame, text=plugin_name, variable=var,
                                     command=lambda pid=plugin_id, v=var: self.on_plugin_toggle(pid, v.get()))
