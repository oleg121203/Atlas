...
class ToolManagementView(ctk.CTkFrame):
    ...
    def populate_tools(self):
        ...
        ctk.CTkLabel(
            header_frame, text="Tool Details", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=0, sticky="w", padx=10)
        ctk.CTkLabel(
            header_frame, text="Success", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=1, sticky="w")
        ctk.CTkLabel(
            header_frame, text="Failure", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=2, sticky="w")
        ctk.CTkLabel(
            header_frame, text="Actions", font=ctk.CTkFont(weight="bold")
        ).grid(row=0, column=3, sticky="w", padx=10)

        ctk.CTkFrame(self.scrollable_frame, height=2, fg_color="gray").pack(
        ...
        if not tools:
            no_tools_label = ctk.CTkLabel(
                self.scrollable_frame, text="No tools are available."
            )
    ...
    def _view_tool_source(self, file_path: str):
        ...
        try:
            ...
            label = ctk.CTkLabel(
                error_dialog, text=f"Error reading file:\n{e}", wraplength=280
            )
    ...
    def _show_tool_info(self, tool: Dict[str, Any]):
        ...
        name_label = ctk.CTkLabel(
            main_frame, text=tool["name"], font=ctk.CTkFont(size=18, weight="bold")
        )
        name_label.pack(pady=(10, 5))

        # Tool type and source
        type_label = ctk.CTkLabel(
            main_frame, text=f"Type: {tool.get('type', 'Unknown').title()}"
        )
        type_label.pack()

        source_label = ctk.CTkLabel(
            main_frame, text=f"Source: {tool.get('source', 'Unknown')}"
        )
        ...
        path_label = ctk.CTkLabel(
            main_frame, text=f"File: {tool.get('file_path', 'N/A')}"
        )
    ...
    def _create_tool_entry(
        self, tool: Dict[str, Any], index: int, usage_stats: Dict[str, int]
    ):
        ...
        name_label = ctk.CTkLabel(
            details_frame, text=tool["name"], font=ctk.CTkFont(weight="bold")
        )
        ...
        doc_label = ctk.CTkLabel(
            details_frame, text=tool["doc"], wraplength=350, justify="left"
        )
        doc_label.pack(anchor="w", padx=5)

        path_label = ctk.CTkLabel(
            details_frame, text=f"Path: {tool['file_path']}", font=ctk.CTkFont(size=10)
        )
        ...
        success_label = ctk.CTkLabel(
            tool_frame, text=str(success_count), font=ctk.CTkFont(size=14)
        )
        ...
        failure_label = ctk.CTkLabel(
            tool_frame, text=str(failure_count), font=ctk.CTkFont(size=14)
        )
    ...
    def _add_creative_templates_section(self):
        ...
        templates_frame.grid(
            row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=(10, 5)
        )
        templates_frame.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(
            templates_frame,
            text="Creative Templates",
        ...
        creative_chains = [
            ...
            (
                "Screenshot → Image Recognition → Notification",
                self._run_screenshot_image_notify,
            ),
            ...
            (
                "Summarize PDF → Translate → Email",
                self._run_summarize_pdf_translate_email,
            ),
    ...
    def _show_feedback_dialog(self, context_msg="Tool chain executed."):
        ...
        ctk.CTkLabel(dialog, text=context_msg, wraplength=380).pack(
            padx=20, pady=(20, 10)
        )
        ...
        def submit():
            print(
                f"[FEEDBACK] Helpful: {helpful_var.get()} | Suggestions: {suggestion_entry.get()}"
            )
            dialog.destroy()
    ...
    def _run_proactive_suggestion(self):
        self.agent_manager.execute_tool(
            "proactive_tool", {"trigger_type": "idle", "threshold": 3}
        )
    ...
    def _open_custom_chain_dialog(self, template_to_load=None):
        ...
        def add_step(tool_name="", args_str=""):
            ...
            tool_dropdown = ctk.CTkComboBox(
                frame, variable=tool_var, values=self._get_tool_names(), width=180
            )
            tool_dropdown.grid(row=0, column=0, padx=5)
            args_entry = ctk.CTkEntry(
                frame, placeholder_text="Arguments (JSON)", width=250
            )
            ...
            remove_btn = ctk.CTkButton(
                ...
                text="Remove",
                width=60,
                command=lambda: remove_step(frame, (tool_var, args_entry)),
            )
        ...
        def save_template():
            ...
            def do_save():
                ...
                for tool_var, args_entry, _ in steps:
                    ...
                    try:
                        args = (
                            eval(args_entry.get()) if args_entry.get().strip() else {}
                        )
                    except Exception:
                        args = {}
                    tool_chain.append({"tool": tool_name, "args": args})
        ...
        def load_template():
            ...
            for t in data:
                ...
                btn = ctk.CTkButton(
                    ...
                    command=load_this_template,
                )
                btn.pack(fill="x", padx=20, pady=3)
            ctk.CTkButton(pick_dialog, text="Cancel", command=pick_dialog.destroy).pack(
                pady=10
            )
        ...
        ctk.CTkLabel(
            dialog,
            text="Custom Tool Chain Composer",
            font=ctk.CTkFont(size=16, weight="bold"),
        ).grid(row=0, column=0, columnspan=4, pady=(10, 5))
        # Steps header
        ctk.CTkLabel(dialog, text="Steps:").grid(row=1, column=0, sticky="w", padx=10)
        # Add Step button
        ctk.CTkButton(dialog, text="Add Step", command=add_step).grid(
            row=1, column=2, sticky="e", padx=10
        )
        # Load Template button
        ctk.CTkButton(dialog, text="Load Template", command=load_template).grid(
        ...
        ctk.CTkButton(dialog, text="Run Chain", command=run_chain).grid(
            row=100, column=0, pady=20, padx=10, sticky="w"
        )
        ctk.CTkButton(dialog, text="Save as Template", command=save_template).grid(
            row=100, column=1, pady=20, padx=10
        )
        ctk.CTkButton(dialog, text="Close", command=dialog.destroy).grid(
            row=100, column=3, pady=20, padx=10, sticky="e"
        )
    ...
    def _run_clipboard_translate_search(self):
        tool_chain = [
            ...
            {"tool": "translate_text", "args": {"target_language": "en"}},
            {
                ...
                "args": {"command": "open https://www.google.com/search?q={input}"},
            },
        ]
        self.agent_manager.execute_tool("creative_tool", {"tool_chain": tool_chain})
    ...
    def _run_screenshot_image_notify(self):
        ...
        self._show_feedback_dialog(
            "Screenshot → Image Recognition → Notification chain executed."
        )
    ...
    def _run_meme_generator_screenshot(self):
        ...
        tool_chain = [
            {"tool": "capture_screen", "args": {}},
            {
                "tool": "add_meme_caption",
                "args": {"caption": "When Atlas automates your day..."},
            },
    ...
    def _run_auto_organize_desktop(self):
        self.agent_manager.execute_tool(
            "proactive_tool", {"trigger_type": "auto_organize_desktop", "threshold": 2}
        )
    ...
    def _run_mute_open_zoom_set_volume(self):
        tool_chain = [
            ...
            {
                "tool": "system_event",
                "args": {"event": "open_app", "app_name": "zoom.us"},
            },
    ...
    def _run_get_apps_open_safari(self):
        tool_chain = [
            ...
            {
                "tool": "system_event",
                "args": {"event": "open_app", "app_name": "Safari"},
            },
        ]
        ToolChainRunnerDialog(self, self.agent_manager, tool_chain)
...
class ToolChainRunnerDialog(ctk.CTkToplevel):
    def __init__(self, parent, agent_manager, tool_chain):
        ...
        ctk.CTkLabel(
            self, text="Tool Chain Execution", font=ctk.CTkFont(size=16, weight="bold")
        ).pack(pady=(10, 5))
        ...
        self.status_label = ctk.CTkLabel(
            self, text="Running...", font=ctk.CTkFont(size=12)
        )
        ...
        self.log_box.tag_configure(
            "success", foreground="#2E8B57", font=("monospace", 11, "bold")
        )
        self.log_box.tag_configure(
            "error", foreground="#B22222", font=("monospace", 11, "bold")
        )
    ...
    async def _run_chain_async(self):
        ...
        for idx, step in enumerate(self.tool_chain):
            ...
            self._log(
                f"Steps succeeded: {self._success_count}, failed: {self._error_count}",
                tag="info",
            )
    ...
    async def _run_tool(self, tool_name, args):
        ...
        if tool:
            ...
            if hasattr(tool_instance, "run") and asyncio.iscoroutinefunction(
                tool_instance.run
            ):
