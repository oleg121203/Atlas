...
def run_tests(with_coverage=False, coverage_target=75, fail_under=70):
    ...
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "--upgrade", "pytest", "pytest-cov", "pytest-qt", "pytest-html"],
        check=False
    )
    ...
    if with_coverage:
        print(f"Running with coverage reporting (target: {coverage_target}%, minimum: {fail_under}%)")
        cmd.extend([
            f"--cov=core",
            f"--cov=tools",
            f"--cov=utils",
...
""")
    ...
    if result.returncode == 0:
        ...
        if with_coverage:
            ...
            print(f"Open 'coverage_html_report/index.html' in a browser to view detailed results")
            ...
            if os.path.exists("coverage.xml"):
                try:
                    ...
                    if 'line-rate' in root.attrib:
                        ...
                        print(f"Overall coverage: {line_rate:.2f}% (target: {coverage_target}%)")
                        if line_rate >= coverage_target:
                            print("🎉 Coverage target met!")
                        else:
                            print(f"⚠️ Coverage below target by {coverage_target - line_rate:.2f}%")
                            print("  Run 'python scripts/test_coverage_enhancer.py' to improve coverage")
...
def main():
    ...
    parser.add_argument('--fail-under', '-f', type=float, default=70.0,
    ...
    if success:
        ...
        if args.coverage:
            ...
            print("Run 'python scripts/test_coverage_enhancer.py' to generate test templates for low-coverage modules.")
    else:
        print("\nPlease fix any failing tests before continuing development.")
        print("You can restore the previous requirements by renaming requirements.txt.bak to requirements.txt")
