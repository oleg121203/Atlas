...
FILE_MAPPINGS = [
    ...
    ('slack_integration.py', 'integration/slack/slack_integration.py', 'integration.slack'),
    
    # Team management files
    ('team_management.py', 'team/team_management.py', 'team'),

    # Analytics files
    ('workflow_analytics_20250626.csv', 'analytics/data/workflow_analytics_20250626.csv', None),
...
def ensure_directory_exists(file_path):
    ...
    if not os.path.exists(dir_path):
        ...
        while True:
            ...
            if not parent_dir.startswith(project_root) or parent_dir == current_dir or parent_dir == project_root:
                break
            ...
            if not os.path.exists(parent_init):
                try:
                    ...
                    print(f"Warning: Permission denied creating {parent_init}, skipping.")
...
def fix_imports_in_file(file_path, old_module_name, new_module_prefix):
    ...
    try:
        ...
        for line in lines:
            ...
            from_import_match = re.match(rf"^from\s+{re.escape(old_module_name)}\s+import\s+(.*)", line)
            ...
            direct_import_match = re.match(rf"^import\s+{re.escape(old_module_name)}(\s*$|\s+as\s+.*$)", line)
            ...
            comma_import_match = re.search(rf"^import\s+(.*?){re.escape(old_module_name)}(.*?)$", line)
            if comma_import_match:
                prefix = comma_import_match.group(1)
                suffix = comma_import_match.group(2)
                new_line = f"import {prefix}{new_module_prefix}.{old_module_name}{suffix}"
                new_lines.append(new_line)
                changed = True
                continue
            ...
            for module in imported_modules:
                ...
                if any(os.path.exists(path) for path in possible_module_paths):
                    ...
                    if re.match(rf"^from\s+{re.escape(module)}\s+import", line):
                        ...
                        rel_prefix = '.' * depth
                        new_line = re.sub(rf"^from\s+{re.escape(module)}\s+", f"from {rel_prefix}{module} ", line)
                        new_lines.append(new_line)
                        line_changed = True
                        changed = True
                        break
                    ...
                    # Handle 'import module'
                    elif re.match(rf"^import\s+{re.escape(module)}(\s*$|\s+as\s+.*$)", line):
                        # Calculate relative import prefix
                        rel_prefix = '.' * depth
                        new_line = re.sub(rf"^import\s+{re.escape(module)}", f"import {rel_prefix}{module}", line)
                        new_lines.append(new_line)
                        line_changed = True
                        changed = True
                        break
...
def move_file_and_fix_imports(src_path, dst_path, module_prefix):
    ...
    for file_path in import_files:
        if os.path.abspath(file_path) != os.path.abspath(dst_path):  # Don't modify the destination file itself
                    ...
                    if import_prefix:
                        for import_file in import_files:
                            if os.path.abspath(import_file) != os.path.abspath(dst_path):
                                fix_imports_in_file(import_file, module_name, import_prefix)
...
def main():
    ...
    try:
        confirm = input("Files have been copied to their new locations. Delete original files? (y/n): ")
        if confirm.lower() == 'y':
            ...
            for pattern, _, _ in TEST_FILES:
                files = glob.glob(os.path.join(ROOT_DIR, pattern))
                for file_path in files:
                    if os.path.isfile(file_path) and os.path.dirname(file_path) == str(ROOT_DIR):
                        os.remove(file_path)
        ...
        print("Skipping file deletion. Original files remain.")
