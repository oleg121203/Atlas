...
def create_sample_data(analytics, num_days=7, workflows=None, executions_per_day=5):
    ...
    for day in range(num_days):
        ...
        for wf in workflows:
            for exec_num in range(executions_per_day):
                start_time = date.replace(hour=random.randint(8, 20), 
                                        minute=random.randint(0, 59), 
                                        second=0) - timedelta(minutes=random.randint(30, 120))
                ...
                error_msg = '' if success else random.choice(['TimeoutError', 'DataValidationError', 'ResourceLimitExceeded'])
                ...
                for i in range(num_steps):
                    step_end = step_start + timedelta(seconds=step_duration + random.randint(-30, 30))
                    step_success = success or (random.random() < 0.9)  # Steps can succeed even if overall fails
                    steps.append({
                        'step_id': f'step_{i+1}',
    ...
    print(f"Created sample data for {len(workflows)} workflows over {num_days} days with {executions_per_day} executions per day.")
    return analytics
...
if __name__ == "__main__":
    ...
    analytics.customizable_dashboard(workflow_ids=['DataPipeline', 'MLTraining', 'ReportGeneration'], export=True)
