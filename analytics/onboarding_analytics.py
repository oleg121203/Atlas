...
class OnboardingAnalytics:
    ...
    def track_step(self, step_name: str, completed: bool = True, duration: Optional[float] = None, error: Optional[str] = None) -> None:
        """
    ...
    def end_session(self, completed_onboarding: bool = True) -> None:
        ...
        self.save_data()
        print(f"Ended onboarding session for user {self.current_session['user_id']}, completed: {completed_onboarding}")
        self.current_session = {}
    ...
    def analyze_drop_off(self) -> Dict[str, Any]:
        ...
        completed_sessions = sum(1 for session in self.data if session.get("completed_onboarding", False))
        ...
        for session in self.data:
            if not session.get("completed_onboarding", False):
                last_step = session["steps"][-1]["step_name"] if session["steps"] else "Start"
...
if __name__ == "__main__":
    ...
    analytics.track_step("Feature Tour", completed=False, duration=15.0, error="User closed window")
