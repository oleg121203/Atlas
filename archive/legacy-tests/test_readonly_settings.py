...
def test_readonly_functionality():
    ...
    settings_view = EnhancedSettingsView(
        ...
        save_callback=lambda settings: print("Save callback called with:", list(settings.keys()))
    )
    ...
    for i, widget in enumerate(settings_view.readonly_widgets[:3]):  # Check first 3 widgets
        try:
