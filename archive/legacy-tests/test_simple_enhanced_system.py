...
def test_self_regeneration_trigger():
    ...
    if initial_result['fixes']:
        ...
        for i, fix in enumerate(initial_result['fixes'], 1):
            print(f"  {i}. {fix['fix_type']}: {fix.get('method', fix.get('file', fix.get('module', 'Unknown')))}")
...
def test_enhanced_email_simulation():
    ...
    print(f"   High priority: {len([e for e in sorted_emails if e['priority'] == 'high'])}")
    print(f"   Medium priority: {len([e for e in sorted_emails if e['priority'] == 'medium'])}")
    print()
...
def main():
    ...
    try:
        ...
        print(f"🔧 Self-regeneration: {regeneration_result['fixes_applied']} fixes applied")
