...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"test_enhanced_email_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
            ),
        ],
    )
...
def test_enhanced_email_system():
    ...
    try:
        ...
        AdaptiveExecutionManager(max_attempts=3)
        ...
        if result.get("success"):
            ...
            if emails_found > 0:
                ...
                for i, email in enumerate(emails, 1):
                    logger.info(
                        f"  {i}. [{email.get('priority', 'unknown')}] {email.get('sender', 'Unknown')}"
                    )
                    logger.info(f"     Subject: {email.get('subject', 'No subject')}")
                    logger.info(f"     Date: {email.get('date', 'Unknown date')}")
                    logger.info(
                        f"     Snippet: {email.get('snippet', 'No snippet')[:100]}..."
                    )
                    logger.info("")
            else:
                logger.warning(
                    "⚠️  FALSE SUCCESS: No emails found despite successful execution"
                )
