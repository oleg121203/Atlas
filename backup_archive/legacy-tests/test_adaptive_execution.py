...
from modules.agents.adaptive_execution_manager import adaptive_execution_manager
...
def test_email_task_adaptation():
    ...
    goal_criteria = {"emails": True, "security_emails": True}
    ...
    result = adaptive_execution_manager.execute_with_adaptation(
        task_description=task_description, goal_criteria=goal_criteria
    )
    ...
    if result.get("adaptation_history"):
        ...
        for i, adaptation in enumerate(result["adaptation_history"], 1):
            print(
                f"  {i}. Attempt {adaptation['attempt_num']}: {adaptation['adaptation_reason']}"
            )
...
def test_browser_task_adaptation():
    ...
    task_description = (
        "Navigate to Gmail using Safari browser and search for security emails"
    )
    goal_criteria = {"navigation": True, "search": True}
    ...
    test_cases = [
        {
            "result": {
                "success": True,
                "data": {"emails": [{"subject": "Security Alert"}]},
            },
        ...
        {"result": {"success": False}, "criteria": {"emails": True}, "expected": False},
    ]

    for i, case in enumerate(test_cases, 1):
        result = adaptive_execution_manager._is_goal_achieved(
            case["result"], case["criteria"]
        )
...
def main():
    ...
    try:
        ...
        total_adaptations = len(email_result.get("adaptation_history", [])) + len(
            browser_result.get("adaptation_history", [])
        )
        print(f"Total Adaptations Made: {total_adaptations}")

        if total_adaptations > 0:
            print(
                "✅ Adaptive execution system is working - strategies are being adapted!"
            )
        else:
