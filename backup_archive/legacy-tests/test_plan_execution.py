...
from modules.agents.hierarchical_plan_manager import (
    ...
    try:
        ...
        plan_manager.current_plan = {
            ...
            "strategic_tasks": len(
                [
                    t
                    for t in plan_manager.tasks.values()
                    if t.level == TaskLevel.STRATEGIC
                ]
            ),
            "tactical_tasks": len(
                [
                    t
                    for t in plan_manager.tasks.values()
                    if t.level == TaskLevel.TACTICAL
                ]
            ),
            "operational_tasks": len(
                [
                    t
                    for t in plan_manager.tasks.values()
                    if t.level == TaskLevel.OPERATIONAL
                ]
            ),
        ...
        print(
            f"   • Operational actions: {plan_manager.current_plan['operational_tasks']}"
        )
...
def display_task_hierarchy(plan_manager, root_task_id, level=0):
    ...
    print(
        f"{indent}{emoji} {level_icon} {task.title} ({task.status.value}){progress_text}"
    )
