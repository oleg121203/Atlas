...
from plugins import get_plugin_manager, register_builtin_plugins, set_active_provider
...
def analyze_duplications():
    ...
    duplications = {
        ...
        "gmail_integration": {
            "components": ["plugins/gmail_plugin.py", "tools/gmail_tool.py"],
            "functionality": [
                "search_emails",
                "get_email_content",
                "authenticate",
...
def test_command_conflicts():
    ...
    # Find conflicts
    conflicts = {
        cmd: plugins for cmd, plugins in all_commands.items() if len(plugins) > 1
    }
...
def main():
    ...
    try:
        ...
        print(
            "This test analyzes compatibility and duplications between plugins and tools."
        )
        ...
        total_duplications = sum(
            len(info["components"]) for info in duplications.values()
        )
        ...
        if total_duplications > 0:
            ...
            print(
                "Recommendation: Follow the recommendations above to resolve duplications."
            )
        else:
