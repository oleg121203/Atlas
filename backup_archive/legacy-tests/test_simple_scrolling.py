...
def test_simple_scrolling():
    ...
    # Add info label
    info_label = ctk.CTkLabel(root, text="Test scrolling with 30 items. Use scroll wheel or scrollbar.")
    ...
    print("Simple scrolling test started. You should see 30 items and be able to scroll to see all of them.")
...
def test_hierarchical_structure():
    ...
    # Create scrollable frame
    scroll_frame = ctk.CTkScrollableFrame(main_frame, label_text="Hierarchical Tasks", height=400)
    scroll_frame.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    for level_name, color, level in levels:
        # Level header
        level_header = ctk.CTkLabel(scroll_frame, text=level_name, 
        ...
        for i in range(8):
            ...
            title_label = ctk.CTkLabel(task_frame, text=title, font=ctk.CTkFont(size=11))
            title_label.pack(side="left", padx=10, pady=5)
            
            # Status
            status_label = ctk.CTkLabel(task_frame, text="●", text_color=color, font=ctk.CTkFont(size=16))
            status_label.pack(side="right", padx=10, pady=5)
