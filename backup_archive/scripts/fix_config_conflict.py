...
def fix_config_conflicts():
    ...
    if env_file.exists():
        ...
        content = content.replace(
            "GROQ_API_KEY=your-groq-key-here", "# GROQ_API_KEY=your-groq-key-here"
        )
        content = content.replace(
        ...
        content = content.replace(
            "DEFAULT_LLM_PROVIDER=gemini", "# DEFAULT_LLM_PROVIDER=gemini"
        )
        content = content.replace(
            "DEFAULT_LLM_MODEL=gemini-1.5-flash", "# DEFAULT_LLM_MODEL=gemini-1.5-flash"
        )
    ...
    print("Next steps:")
    print(
        "1. Edit ~/.atlas/config.yaml and replace 'gsk_your-actual-groq-key-here' with your real Groq API key"
    )
...
def show_current_priority():
    ...
    print(
        f"Groq API Key: {config_manager.get_groq_api_key()[:10] if config_manager.get_groq_api_key() else 'Not set'}..."
    )
    print(
        f"Gemini API Key: {config_manager.get_gemini_api_key()[:10] if config_manager.get_gemini_api_key() else 'Not set'}..."
    )
