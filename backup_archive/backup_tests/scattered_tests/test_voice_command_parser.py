...
if not PY_AUDIO_AVAILABLE:
    print(
        "PyAudio or speech_recognition not available. Voice command functionality will be disabled."
    )

# Import after checking module availability
with patch.dict(
    "sys.modules",
    {
        "pyaudio": None if not PY_AUDIO_AVAILABLE else MagicMock(),
        "speech_recognition": None if not PY_AUDIO_AVAILABLE else MagicMock(),
    },
) as mocked_modules:
    if not PY_AUDIO_AVAILABLE:
