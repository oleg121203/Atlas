...
class TestContextualPromptGenerator(unittest.TestCase):
    def setUp(self):
        ...
        history_data = [
            ...
            {"user_id": "other_user", "workflow": {"name": "Other Flow", "steps": []}},
        ]
        ...
        dashboard_data = {
            ...
            "metrics": {"data_points": 1500, "last_updated": "2023-05-10"},
        }
    ...
    def test_load_user_history(self):
        ...
        test_user_history = [
            h for h in self.generator.user_history if h["user_id"] == "test_user"
        ]
    ...
    def test_load_dashboard_configs(self):
        ...
        self.assertEqual(
            self.generator.dashboard_configs["active_dashboards"][0], "Sales Dashboard"
        )
    ...
    def test_generate_contextual_prompt_default_user(self):
        ...
        self.assertIn(
            "You are assisting a User with workflow creation", enhanced_prompt
        )
        self.assertIn(
            "User request: Create a workflow for data analysis", enhanced_prompt
        )

    def test_generate_contextual_prompt_test_user(self):
        """Test generating contextual prompt for specific test user"""
        base_prompt = "Create a workflow for data analysis"
        enhanced_prompt = self.generator.generate_contextual_prompt(
            base_prompt, user_id="test_user"
        )

        self.assertIn(
            "You are assisting a Data Analyst with workflow creation", enhanced_prompt
        )
        self.assertIn(
            "User preferences: style=concise, complexity=intermediate, domain=data processing",
            enhanced_prompt,
        )
        self.assertIn("Frequently used actions: extract_data", enhanced_prompt)
        self.assertIn(
            "Active dashboards: Sales Dashboard, Performance Metrics", enhanced_prompt
        )
        self.assertIn(
            "User request: Create a workflow for data analysis", enhanced_prompt
        )
    ...
    def test_generate_contextual_prompt_with_context_data(self):
        ...
        enhanced_prompt = self.generator.generate_contextual_prompt(
            base_prompt, user_id="test_user", context_data=context_data
        )
