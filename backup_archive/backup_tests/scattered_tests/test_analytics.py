...
class TestWorkflowAnalytics(unittest.TestCase):
    ...
    def test_start_and_end_execution(self):
        ...
        self.analytics.end_execution(
            self.execution_id, "completed", "All actions successful"
        )
    ...
    def test_record_action(self):
        ...
        self.analytics.record_action(
            self.execution_id, "action1", start_time, end_time, "success"
        )
    ...
    def test_record_failed_action(self):
        ...
        self.analytics.end_execution(
            self.execution_id, "failed", "Workflow failed due to action timeout"
        )
    ...
    def test_analyze_failures(self):
        ...
        for i in range(3):
            ...
            self.analytics.record_action(
                exec_id, "connect_action", start_time, end_time, "failed", error_msg
            )
            self.analytics.end_execution(
                exec_id, "failed", f"Failed due to {error_msg}"
            )
    ...
    def test_optimize_workflow(self):
        ...
        for i in range(3):
            ...
            end_time = datetime.now()
            self.analytics.record_action(
                exec_id, "long_action", start_time, end_time, "success"
            )
            start_time = datetime.now()
            time.sleep(0.1)  # Short action
            end_time = datetime.now()
            self.analytics.record_action(
                exec_id, "short_action", start_time, end_time, "success"
            )
        ...
        self.assertTrue(
            any(
                "long_action" in bottleneck for bottleneck in suggestions["bottlenecks"]
            )
        )
        self.assertGreaterEqual(len(suggestions["parallelization_opportunities"]), 1)
        self.assertTrue(
            any(
                "long_action" in opportunity
                for opportunity in suggestions["parallelization_opportunities"]
            )
        )
