...
def test_formatting_functions():
    ...
    tests = [
        ...
        (
            "Link",
            lambda t: manager.format_link(t, "http://example.com"),
            f"[{test_text}](http://example.com)",
        ),
    ]
    ...
    for text, expected_lang in test_texts:
        try:
            ...
            if detected_lang == expected_lang:
                print(
                    f"    ✅ '{text}' -> {detected_lang} (confidence: {confidence:.2f})"
                )
            else:
                print(
                    f"    ⚠️ '{text}' -> Expected {expected_lang}, got {detected_lang} (confidence: {confidence:.2f})"
                )
        except Exception as e:
            print(f"    ❌ Помилка: {e}")
            return False
