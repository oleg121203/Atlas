...
def create_workflow_test_suite():
    ...
    class WorkflowStepTests(unittest.TestCase):
        def setUp(self):
            self.test_executions = framework.generate_test_data(
                "workflow_execution", count=10
            )
        ...
        def test_step_execution_success_rate(self):
            ...
            self.assertGreaterEqual(
                success_rate, 0.7, "Workflow success rate below 70%"
            )
        ...
        def test_user_roles(self):
            ...
            for user in self.test_users:
                self.assertIn(
                    user["role"],
                    valid_roles,
                    f"Invalid role for user {user['user_id']}",
                )

        def test_external_api_call(self):
            from external_api.service import (
                get_data,  # This would normally be a real import
            )
    ...
    class WorkflowIntegrationTests(unittest.TestCase):
        ...
        def test_workflow_feedback_distribution(self):
            ...
            self.assertGreaterEqual(
                avg_nps, 5.0, "Average NPS score below acceptable threshold"
            )
