...
def test_default_provider():
    ...
    print(
        f"🤖 LLM Manager: provider={llm_manager.current_provider}, model={llm_manager.current_model}"
    )
    print(
        f"🔌 OpenAI client: {'Available' if llm_manager.is_provider_available('openai') else 'Not available'}"
    )
    print(
        f"💎 Gemini client: {'Available' if llm_manager.gemini_client else 'Not available'}"
    )
