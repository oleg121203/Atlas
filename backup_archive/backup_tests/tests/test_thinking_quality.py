...
def test_thinking_quality():
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        try:
            ...
            print(
                f"📊 Контекст: домен={context.domain}, складність={context.complexity_level}/5"
            )
            ...
            print(
                f"🎯 Стратегія: {strategy.value} {'✅' if strategy_correct else '❌'}"
            )

            # Фаза 3: Generation питань
            questions = tool.generate_strategic_questions(
                test["query"], strategy, context
            )
    ...
    for result in results:
        if "error" not in result:
            ...
            print(
                f"• {result['category']}: {result['quality_score']}% {strategy_status}"
            )
        else:
...
def test_meta_cognition():
    ...
    complex_query = (
        "Як система мета-когнітивного аналізу в Atlas може самовдосконалюватися?"
    )
