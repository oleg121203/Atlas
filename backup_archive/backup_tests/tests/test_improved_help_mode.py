...
try:
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        if mode_detected == test["expected_mode"]:
            ...
            print(
                f"   ❌ Expected: {test['expected_mode']}, Got: {mode_detected} (confidence: {confidence:.2f})"
            )
        ...
        print(
            f"   📋 Prompt encourages tool usage: {'✅ Yes' if uses_tools else '❌ No'}"
        )
