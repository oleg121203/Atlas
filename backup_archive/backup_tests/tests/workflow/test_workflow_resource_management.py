...
from workflow.workflow_resource_management import (
    PriorityLevel,
    ResourceType,
...
class TestWorkflowResourceManager(unittest.TestCase):
    def setUp(self):
        ...
        self.manager.register_resource(
            self.resource_id, ResourceType.CPU, 8.0, "cores", "data_center_1", 0.10
        )
    ...
    def test_allocate_resource(self):
        ...
        result = self.manager.allocate_resource(
            self.workflow_id, self.resource_id, 2.0, 1.0
        )
    ...
    def test_allocate_resource_insufficient_capacity(self):
        ...
        result = self.manager.allocate_resource(
            self.workflow_id, self.resource_id, 10.0, 1.0
        )
    ...
    def test_schedule_workflow(self):
        ...
        result = self.manager.schedule_workflow(
            self.workflow_id, start_time, 2.0, required_resources, PriorityLevel.HIGH
        )
        self.assertTrue(result)
        ...
        self.assertEqual(
            self.manager.priority_queue[self.workflow_id], PriorityLevel.HIGH
        )
    ...
    def test_optimize_costs(self):
        ...
        result = self.manager.optimize_costs(
            self.workflow_id, cloud_resources, 0.25, performance_reqs
        )
    ...
    def test_get_workflow_schedule(self):
        ...
        self.manager.schedule_workflow(
            self.workflow_id, start_time, 2.0, required_resources, PriorityLevel.MEDIUM
        )
