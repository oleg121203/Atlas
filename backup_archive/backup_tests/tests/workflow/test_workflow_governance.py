...
from workflow.workflow_governance import (
    AccessRole,
    ComplianceStandard,
    WorkflowGovernance,
)
...
class TestWorkflowGovernance(unittest.TestCase):
    ...
    def test_version_workflow(self):
        ...
        self.governance.version_workflow(
            self.workflow_id, self.workflow_data, self.version
        )
        ...
        self.assertEqual(
            self.governance.workflow_versions[self.workflow_id][0]["version"],
            self.version,
        )
    ...
    def test_request_approval(self):
        ...
        result = self.governance.request_approval(
            self.workflow_id, self.version, self.user_id, "Test request"
        )
        ...
        self.assertEqual(
            self.governance.approval_records[self.workflow_id][0]["status"], "pending"
        )
    ...
    def test_approve_workflow(self):
        ...
        result = self.governance.approve_workflow(
            self.workflow_id, self.version, "approver", "approved", "Looks good"
        )
        self.assertTrue(result)
        self.assertEqual(
            self.governance.approval_records[self.workflow_id][0]["status"], "approved"
        )
    ...
    def test_reject_workflow(self):
        ...
        self.governance.request_approval(self.workflow_id, self.version, self.user_id)
        result = self.governance.approve_workflow(
            self.workflow_id, self.version, "approver", "rejected", "Needs revision"
        )
        self.assertTrue(result)
        self.assertEqual(
            self.governance.approval_records[self.workflow_id][0]["status"], "rejected"
        )
    ...
    def test_audit_trail_logging(self):
        ...
        self.assertEqual(
            self.governance.audit_trail[self.workflow_id][0]["event_type"],
            "version_created",
        )
    ...
    def test_compliance_check_gdpr(self):
        ...
        result = self.governance.run_compliance_check(
            self.workflow_id, ComplianceStandard.GDPR, non_compliant_data
        )
    ...
    def test_compliance_check_hipaa(self):
        ...
        result = self.governance.run_compliance_check(
            self.workflow_id, ComplianceStandard.HIPAA, non_compliant_data
        )
        self.assertFalse(result["compliant"])
        self.assertGreater(len(result["issues"]), 0)
    ...
    def test_role_based_access_control(self):
        ...
        self.governance.configure_role_access(
            self.workflow_id, AccessRole.EDITOR, permissions
        )
        self.governance.assign_user_role(
            self.user_id, self.workflow_id, AccessRole.EDITOR
        )
        access_result = self.governance.check_user_access(
            self.user_id, self.workflow_id, "edit"
        )
        self.assertTrue(access_result)
        access_result = self.governance.check_user_access(
            self.user_id, self.workflow_id, "delete"
        )
