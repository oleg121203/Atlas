...
class TestWorkflowAnalytics(unittest.TestCase):
        ...
        self.steps = [
            {
                ...
                "end_time": self.start_time + timedelta(minutes=2),
                "success": True,
            },
            {
                "step_id": "step2",
    ...
    def test_record_execution_success(self):
        self.analytics.record_execution(
            "wf1", "exec1", self.start_time, self.end_time, True, "", self.steps
        )
    ...
    def test_record_execution_failure(self):
        self.analytics.record_execution(
            "wf1", "exec2", self.start_time, self.end_time, False, "Timeout error"
        )
        self.assertEqual(len(self.analytics.execution_data), 1)
        self.assertEqual(self.analytics.execution_data.iloc[0]["success"], False)
        self.assertEqual(
            self.analytics.execution_data.iloc[0]["error_message"], "Timeout error"
        )
    ...
    def test_get_performance_metrics(self):
        ...
        self.analytics.record_execution(
            ...
            self.end_time - timedelta(hours=1),
            False,
            "Error",
        )
    ...
    def test_comparative_analytics(self):
        ...
        self.analytics.record_execution(
            "wf2", "exec2", self.start_time, self.end_time, False, "Error"
        )
        ...
        self.assertEqual(
            comparison[comparison["workflow_id"] == "wf1"]["success_rate"].iloc[0],
            100.0,
        )
        self.assertEqual(
            comparison[comparison["workflow_id"] == "wf2"]["success_rate"].iloc[0], 0.0
        )

    def test_comparative_analytics_empty(self):
        comparison = self.analytics.comparative_analytics(
            entity_type="workflow", entity_ids=["wf3"]
        )
    ...
    def test_predict_workflow_failure_untrained(self):
        prediction = self.analytics.predict_workflow_failure(
            "wf1", 300.0, datetime.now().weekday()
        )
