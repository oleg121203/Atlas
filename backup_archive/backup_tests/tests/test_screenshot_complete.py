...
def test_imports():
    ...
    if importlib.util.find_spec("Quartz"):
        ...
        return True
    print(
        "❌ Quartz: Not available. This is expected if not on macOS or if PyObjC is not installed."
    )
    return False
...
def test_native_screencapture():
    ...
    try:
        ...
        result = subprocess.run(
            ["which", "screencapture"], check=False, capture_output=True, text=True
        )
...
def test_quartz_capture():
    ...
    try:
        ...
        print(
            f"✅ PIL Image created: {img.size[0]}x{img.size[1]} pixels, mode: {img.mode}"
        )
...
def test_applescript_capture():
    ...
    try:
        ...
        result = subprocess.run(
            ["which", "osascript"], check=False, capture_output=True, text=True
        )
        if result.returncode != 0:
...
def test_integrated_screenshot():
    ...
    try:
        ...
        img = capture_screen()
        print(
            f"✅ Integrated screenshot: {img.size[0]}x{img.size[1]} pixels, mode: {img.mode}"
        )
