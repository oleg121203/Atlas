...
def test_auto_mode_detection():
    ...
    for message, expected_mode, description in test_cases:
        ...
        print(
            f"    → {context.mode.value} (confidence: {context.confidence:.2f}) - {description}"
        )
        print()
...
def test_development_mode():
    ...
    for message in dev_messages:
        context = context_manager.analyze_message(message, system_info)
        response_prompt = context_manager.generate_response_prompt(
            context, message, system_info
        )
...
def test_mode_persistence():
    ...
    # Auto mode
    states.append(
        ("Auto mode", context_manager.is_auto_mode, context_manager.current_mode)
    )

    # Manual chat
    context_manager.set_manual_mode(ChatMode.CASUAL_CHAT)
    states.append(
        ("Manual chat", context_manager.is_auto_mode, context_manager.current_mode)
    )

    # Manual development
    context_manager.set_manual_mode(ChatMode.DEVELOPMENT)
    states.append(
        ("Manual dev", context_manager.is_auto_mode, context_manager.current_mode)
    )

    # Back to auto
    context_manager.toggle_auto_mode()
    states.append(
        ("Back to auto", context_manager.is_auto_mode, context_manager.current_mode)
    )
