...
class ModuleErrorHandlingTest(unittest.TestCase):
    ...
    def test_module_initialization_with_failures(
        self,
        mock_self_improvement,
        mock_system,
        mock_stats,
        mock_settings,
        mock_plugins,
        ...
        mock_self_improvement.side_effect = Exception(
            "SelfImprovementCenter initialization failed"
        )
        ...
        self.assertIn(
            "chat",
            main_window.modules,
            "Chat module should be initialized with fallback",
        )
        self.assertIsInstance(
            main_window.modules["chat"],
            QWidget,
            "Chat module should have fallback widget",
        )
        self.assertIn(
            "agents", main_window.modules, "Agents module should be initialized"
        )
        self.assertNotIsInstance(
            main_window.modules["agents"],
            QWidget,
            "Agents module should not be a fallback",
        )
        self.assertIn(
            "tasks",
            main_window.modules,
            "Tasks module should be initialized with fallback",
        )
        self.assertIsInstance(
            main_window.modules["tasks"],
            QWidget,
            "Tasks module should have fallback widget",
        )
        self.assertIn(
            "plugins", main_window.modules, "Plugins module should be initialized"
        )
        self.assertNotIsInstance(
            main_window.modules["plugins"],
            QWidget,
            "Plugins module should not be a fallback",
        )
        self.assertIn(
            "settings",
            main_window.modules,
            "Settings module should be initialized with fallback",
        )
        self.assertIsInstance(
            main_window.modules["settings"],
            QWidget,
            "Settings module should have fallback widget",
        )
        self.assertIn(
            "stats", main_window.modules, "Stats module should be initialized"
        )
        self.assertNotIsInstance(
            main_window.modules["stats"],
            QWidget,
            "Stats module should not be a fallback",
        )
        self.assertIn(
            "system", main_window.modules, "System module should be initialized"
        )
        self.assertNotIsInstance(
            main_window.modules["system"],
            QWidget,
            "System module should not be a fallback",
        )
        self.assertTrue(
        ...
        self.assertIsInstance(
            main_window.self_improvement_module,
            QWidget,
            "SelfImprovement module should have fallback widget",
        )
    ...
    def test_module_initialization_without_meta_agent(self):
        ...
        self.assertIn(
            "tasks", main_window.modules, "Tasks module should be initialized"
        )
        self.assertIn(
            "plugins", main_window.modules, "Plugins module should be initialized"
        )
        self.assertIn(
            "settings", main_window.modules, "Settings module should be initialized"
        )
        self.assertIn(
            "stats", main_window.modules, "Stats module should be initialized"
        )
    ...
    def test_plugin_manager_connection_failure(self, mock_plugin_manager):
        ...
        mock_plugin_manager.side_effect = Exception(
