...
class TestChatModule:
    ...
    def test_message_sending(self, qapp):
        ...
        self.chat_module.input_field.keyPressEvent(MagicMock(key=Qt.Key.Key_Enter))

        # Verify message was sent
        self.mock_agent_manager.send_message.assert_called_once_with("Test message")

        # Verify message appears in chat
        assert self.chat_module.chat_view.count() > 0
        last_message = self.chat_module.chat_view.item(
            self.chat_module.chat_view.count() - 1
        )
    ...
    def test_message_history(self, qapp):
        ...
        assert (
            self.chat_module.chat_view.verticalScrollBar().value()
            == self.chat_module.chat_view.verticalScrollBar().maximum()
        )
    ...
    def test_search_functionality(self, qapp):
        ...
        visible_items = [
            i
            for i in range(self.chat_module.chat_view.count())
    ...
    def test_theme_changes(self, qapp):
        ...
        assert self.chat_module.chat_view.styleSheet().startswith(
