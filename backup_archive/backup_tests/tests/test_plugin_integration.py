...
def test_plugin_integration():
    ...
    try:
        ...
        if result and "tools" in result and result["tools"]:
            ...
            if (
                hasattr(mock_app, "advanced_ai_thinking_integration")
                and mock_app.advanced_ai_thinking_integration
            ):
        ...
        if success:
            ...
            for query in test_queries:
                try:
                    ...
                    is_advanced = (
                        "проаналізуй" in query.lower()
                        or "покращ" in query.lower()
                        or "що не так" in query.lower()
                    )

                    if is_advanced and "Original handler" in response:
                        issues.append(
                            f"⚠️  Складний запит '{query}' обробляється простим handler"
                        )
...
def analyze_current_detection_system():
    ...
    try:
        ...
        advanced_thinking_file = (
            base_dir / "plugins" / "helper_sync_tell" / "advanced_thinking.py"
        )
