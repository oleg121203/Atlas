...
try:
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        print(
            f"   ✅ Mentioned tools: {', '.join(mentioned_tools) if mentioned_tools else 'None'}"
        )
        ...
        if "problem" in test["message"].lower() or "помилки" in test["message"].lower():
            has_professional_mode = (
                "PROFESSIONAL ANALYSIS MODE" in prompt
                or "Professional Code Analyzer" in prompt
            )
            print(
                f"   🎓 Professional mode: {'✅ Activated' if has_professional_mode else '❌ Not activated'}"
            )
