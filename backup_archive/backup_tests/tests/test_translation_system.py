...
def test_language_detection():
    ...
    for text, expected_lang in test_cases:
        ...
        print(
            f"{status} '{text}' -> {detected_lang} (confidence: {confidence:.2f}, expected: {expected_lang})"
        )
...
def test_translation_context():
    ...
    class MockLLMManager:
        def chat(self, messages):
            ...
            if (
                "Hello" in user_msg
                and "translate to Ukrainian" in messages[0]["content"]
            ):
    ...
    processed_msg2, context2 = translation_manager.process_incoming_message(
        english_message
    )
...
def test_translation_status():
    ...
    for msg in test_messages:
        info = (
            translation_tool.detect_language_info(msg)
            if hasattr(translation_tool, "detect_language_info")
            else {}
        )
        print(f"Message: {msg}")
        if info:
...
def main():
    ...
    try:
        ...
        print(
            "Users can now chat in Ukrainian or Russian, and responses will be automatically translated."
        )
