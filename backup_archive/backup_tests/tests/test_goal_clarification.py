...
class TestGoalClarification(unittest.TestCase):
    ...
    def test_clear_goal_does_not_trigger_clarification(self):
        ...
        mock_response = LLMResponse(
            response_text=json.dumps({"is_ambiguous": False, "question": ""}),
            model="",
            prompt_tokens=0,
            completion_tokens=0,
            total_tokens=0,
        ...
        print(
            f"Agent state before run_once - is_running: {self.master_agent.is_running}, is_paused: {self.master_agent.is_paused}"
        )

        # Mock the execution to prevent further processing
        with patch.object(
            self.master_agent, "_execute_objective_with_retries", return_value=None
        ) as mock_execute:
            ...
            self.assertFalse(
                clarification_call_found,
                "Clarification should not be requested for a clear goal.",
            )
    ...
    def test_ambiguous_goal_triggers_clarification(self):
        ...
        clarification_question = (
            "Which document do you mean, and what does 'process' entail?"
        )
        # Mock the LLM to return an ambiguous response
        mock_response = LLMResponse(
            response_text=json.dumps(
                {"is_ambiguous": True, "question": clarification_question}
            ),
            model="",
            prompt_tokens=0,
            completion_tokens=0,
        ...
        self.assertEqual(
            self.master_agent.clarification_question, clarification_question
        )
        self.assertTrue(self.master_agent.is_paused)
        self.mock_status_callback.assert_any_call(
            {"type": "request_clarification", "content": clarification_question}
        )
    ...
    def test_agent_resumes_with_clarified_goal(self):
        ...
        user_clarification = (
            "I want you to summarize the text content of '/path/to/my/file.txt'"
        )
        ...
        ambiguous_response = LLMResponse(
            response_text=json.dumps({"is_ambiguous": True, "question": "Which file?"}),
            model="",
            prompt_tokens=0,
            completion_tokens=0,
            total_tokens=0,
