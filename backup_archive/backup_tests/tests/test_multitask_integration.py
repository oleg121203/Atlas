...
class MockLLMManager:
    ...
    def chat(self, messages):
        ...
        if "status" in user_msg:
            return MockResult(
                '["Check system processes", "Check disk space", "Check memory usage"]'
            )
...
class MultitaskIntegrationTest:
    ...
    def run_all_tests(self):
        ...
        for test_name, test_func in tests:
            ...
            try:
                ...
                print(
                    f"{status_icon} {test_name}: {self.results[test_name]['status']} ({duration:.2f}s)"
                )
    ...
    def test_memory_isolation(self) -> bool:
        ...
        try:
            ...
            for task_id, task_data in task_memories.items():
                ...
                print(
                    f"   📝 Task {task_id}: {len(memories)} memories in scope '{scope}'"
                )
                ...
                for other_task_id, other_data in task_memories.items():
                    if other_task_id != task_id:
                        ...
                        assert scope != other_scope, (
                            f"Memory scopes should be unique: {scope} vs {other_scope}"
                        )
            ...
            for task_id, task_data in task_memories.items():
                ...
                retrieved = [
                    m
                    for m in task_data["memories"]
                    if ("screenshot" in str(m) and task_id == "task_1")
                    or ("weather" in str(m) and task_id == "task_2")
                    or ("status" in str(m) and task_id == "task_3")
                ]

                print(
                    f"   🔍 Task {task_id}: Retrieved {len(retrieved)} relevant memories"
                )
    ...
    def test_concurrent_execution(self) -> bool:
        ...
        try:
            ...
            # Analyze execution
            completed_tasks = [
                log for log in task_execution_log if log["event"] == "completed"
            ]
    ...
    def test_api_resource_management(self) -> bool:
        ...
        try:
            ...
            class MockAPIResourceManager:
                ...
                def can_make_request(self, provider: str) -> bool:
                    ...
                    self.counters[provider] = [
                        req_time
                        for req_time in self.counters[provider]
            ...
            assert openai_stats["current"] >= openai_stats["limit"], (
                "OpenAI should be at/over limit"
            )
            print(
                f"   📊 OpenAI usage: {openai_stats['current']}/{openai_stats['limit']}"
            )

            ollama_stats = stats["ollama"]
            assert ollama_stats["current"] < ollama_stats["limit"], (
                "Ollama should be under limit"
            )
            print(
                f"   📊 Ollama usage: {ollama_stats['current']}/{ollama_stats['limit']}"
            )
    ...
    def _print_summary(self):
        ...
        passed_tests = len(
            [r for r in self.results.values() if r["status"] == "PASSED"]
        )
        failed_tests = len(
