...
class TestStrategicPlanner:
    ...
    def test_generate_strategic_plan_success_with_numbered_list(
        self, mock_llm_manager, mock_memory_manager
    ):
        ...
        planner = StrategicPlanner(
            llm_manager=mock_llm_manager, memory_manager=mock_memory_manager
        )
    ...
    def test_generate_strategic_plan_fallback_parsing(
        self, mock_llm_manager, mock_memory_manager
    ):
        """Test fallback parsing when the response is not a numbered list."""
        # Arrange
        mock_response = "Your Output:\nFirst objective.\nSecond objective."
        mock_llm_manager.chat.return_value.response_text = mock_response
        planner = StrategicPlanner(
            llm_manager=mock_llm_manager, memory_manager=mock_memory_manager
        )
    ...
    def test_generate_strategic_plan_empty_response(
        self, mock_llm_manager, mock_memory_manager
    ):
    ...
    def test_generate_strategic_plan_llm_error(
        self, mock_llm_manager, mock_memory_manager
    ):
