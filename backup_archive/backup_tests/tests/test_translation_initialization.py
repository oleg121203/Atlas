...
def test_translation_initialization():
    ...
    try:
        ...
        detected_lang, confidence = (
            translation_manager.translation_tool.detect_language(ukrainian_text)
        )
        print(
            f"   Детекція мови: '{ukrainian_text}' -> {detected_lang} (впевненість: {confidence:.2f})"
        )

        # Verification чи потрібен переклад
        should_translate = (
            translation_manager.translation_tool.should_translate_message(
                ukrainian_text
            )
        )
        ...
        # Тест перекладу
        processed_msg, context = translation_manager.process_incoming_message(
            ukrainian_text
        )
        print(f"   Оброблене повідомлення: '{processed_msg}'")
        print(f"   Мова користувача: {context.user_language}")
        print(
            f"   Потрібен переклад відповіді: {context.requires_response_translation}"
        )
        ...
        if context.requires_response_translation:
            translated_response = translation_manager.process_outgoing_response(
                english_response
            )
...
def test_language_detection_patterns():
    ...
    try:
        ...
        for phrase, expected_lang, description in test_phrases:
            detected_lang, confidence = (
                translation_manager.translation_tool.detect_language(phrase)
            )
            should_translate = (
                translation_manager.translation_tool.should_translate_message(phrase)
            )
