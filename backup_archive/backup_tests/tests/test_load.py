...
class ChatLoadTest(AtlasLoadTest):
    ...
    def test_chat_operations(self):
        ...
        try:
            ...
            messages = [
                {"role": "user", "content": f"Test message {i}"} for i in range(1000)
            ]
...
@pytest.mark.parametrize(
    "test_class", [PluginLoadTest, ChatLoadTest, TaskLoadTest, UILoadTest]
)
def test_load(test_class):
    ...
    assert all(
        rt < 1.0 for rt in test.metrics["response_times"]
