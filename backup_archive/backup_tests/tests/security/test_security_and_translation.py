        ...
        def chat(self, messages):
            ...
            if "translate" in system_prompt.lower():
                ...
                if "Development mode" in content:
                    return MockResult(
                        "Режим розробки - Розширений системний доступ увімкнено..."
                    )
                if "For my dear creator" in content:
                    return MockResult("Для мого дорогого творця та батька:")
                if "I understand this as a goal" in content:
                    return MockResult(
                        "Я розумію це як мету. Дозвольте мені працювати над цим..."
                    )
                if "Privileged access activated" in content:
