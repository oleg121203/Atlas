...
def test_decompose_goal_successful_path(decomposition_agent, mock_llm_manager):
    ...
    """
    complex_goal = (
        "Develop a go-to-market strategy for a new AI-powered code assistant."
    )
    ...
    result_path = decomposition_agent.decompose_goal(
        complex_goal, max_depth=2, breadth=3
    )
    ...
    assert (
        result_path[0]
        == "Identify target developer segments (e.g., enterprise, startup, student)."
    )
...
def test_decompose_goal_no_viable_thoughts(decomposition_agent, mock_llm_manager):
    ...
    def mock_chat_router(messages):
        ...
        if "Thought to Evaluate" in user_prompt:
            return LLMResponse(
                "0.1", "mock_model", 10, 1, 11
            )  # All thoughts score very low
    ...
    assert result_path is None, (
        "Result should be None when no viable thoughts are found"
    )
...
def test_decompose_goal_max_depth_reached(decomposition_agent, mock_llm_manager):
    ...
    def mock_chat_router(messages):
        ...
        if "Thought to Evaluate" in user_prompt:
            return LLMResponse(
                "0.9", "mock_model", 10, 1, 11
            )  # High score for all thoughts
    ...
    result_path = decomposition_agent.decompose_goal(
        complex_goal, max_depth=1, breadth=2
    )
...
def test_decompose_goal_breadth_pruning(decomposition_agent, mock_llm_manager):
    ...
    assert result_path is not None, "Result should not be None"
    assert result_path[0] == "Best thought.", (
        "Should select the highest-scoring thought"
    )
    assert mock_llm_manager.chat.call_count <= 6, (
        "Should evaluate only top 'breadth' thoughts"
    )
