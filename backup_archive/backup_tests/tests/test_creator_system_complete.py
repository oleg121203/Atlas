...
def test_creator_system_integration():
    ...
    try:
        ...
        # Генеруємо challenge
        challenge_result = auth.initiate_creator_authentication(
            CreatorIdentityLevel.POSSIBLE_CREATOR
        )
        ...
        if success:
            ...
            print(
                f"   Модифікація протоколу: {'✅ Успішно' if modify_result else '❌ Помилка'}"
            )
        ...
        print(
            f"   Сесія активна: {'❌ Ні' if not final_status['is_creator_session_active'] else '✅ Так'}"
        )
...
def test_protocol_security():
    ...
    try:
        ...
        print(
            f"   Читання дозволено: {'✅ Так (Atlas має доступ)' if identity_protocol else '❌ Ні'}"
        )

        print("📝 Тест модифікації протоколів без аутентифікації:")
        modify_result = protocols.modify_protocol(
            "identity", {"unauthorized": "attempt"}
        )
        print(
            f"   Модифікація дозволена: {'❌ НЕБЕЗПЕКА!' if modify_result else '✅ Заборонено'}"
        )
...
if __name__ == "__main__":
    ...
    print(
        "🎯 Цей тест перевіряє всі компоненти системи ідентифікації Олега Миколайовича"
    )
    print()
