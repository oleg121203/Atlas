...
class TestWebSocketLatencyConflict(unittest.TestCase):
    ...
    def create_client(self, user_id):
        ...
        client = WebSocketClient("ws://localhost:8765", "default_team", None)
        return client
        ...
        for _ in range(20):  # Wait up to 2 seconds
            if hasattr(user2, "received_messages") and user2.received_messages:
                for msg in user2.received_messages:
                    if (
                        msg.get("type") == "task_update"
                        and msg.get("data", {}).get("id") == "task1"
                    ):
                        received = True
                        break
            if received:
    ...
    def test_conflict_resolution_timestamp(self):
        ...
        task1_update1 = {
            "type": "task_update",
            "data": {"id": "task2", "status": "update1", "timestamp": time.time() - 1},
        }
        task1_update2 = {
            "type": "task_update",
            "data": {"id": "task2", "status": "update2", "timestamp": time.time()},
        }
        ...
        for client in self.clients:
            if hasattr(client, "received_messages") and client.received_messages:
                for msg in client.received_messages:
                    if (
                        msg.get("type") == "task_update"
                        and msg.get("data", {}).get("id") == "task2"
                    ):
                        data = msg.get("data", {})
                        if final_update is None or data.get(
                            "timestamp", 0
                        ) > final_update.get("timestamp", 0):
                            final_update = data

        self.assertIsNotNone(final_update, "Should have received at least one update")
        self.assertEqual(
            final_update.get("status"),
            "update2",
            "Later timestamp update should take precedence",
        )
    ...
    def test_conflict_resolution_multiple_updates(self):
        ...
        self.assertIsNotNone(final_update, "Should have received at least one update")
        self.assertTrue(
            final_update.get("status", "").startswith("update2"),
            "Later updates should take precedence",
        )
    ...
    def test_multiple_clients_update_same_task(self):
        try:
            ...
            self.assertEqual(
                last_update["status"], "updated_by_client_2"
            )  # Last client update
