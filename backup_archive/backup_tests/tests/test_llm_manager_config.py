...
def test_llm_manager():
    ...
    try:
        ...
        provider, model = llm_manager._validate_provider_model(
        ...
        provider, model = llm_manager._validate_provider_model(
            "invalid_provider", "invalid_model"
        )
    ...
    if "gemini" in providers and llm_manager.gemini_client:
        try:
            ...
            messages = [
                {
                    "role": "user",
                    "content": "Say 'Hello from Atlas!' in exactly those words.",
                }
            ]
            response = llm_manager.chat(
                messages, provider="gemini", model="gemini-1.5-flash"
            )
