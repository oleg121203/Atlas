...
class TestSelfLearningAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.agent = SelfLearningAgent(
            memory_manager=self.memory_manager, model_path="test_models"
        )
    ...
    def test_adapt_response_with_model(self, mock_scaler, mock_regressor):
        ...
        self.assertEqual(
            result["response"],
            candidates[1],
            "Should select the response with higher predicted reward",
        )
    ...
    def test_reset_learning_data(self):
        ...
        self.agent.collect_feedback(
            "another_user", "Another query", "Another response", 3.0
        )
