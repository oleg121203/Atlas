...
class ModuleCommunicationTest(unittest.TestCase):
    ...
    def test_module_event_registration(self):
        ...
        self.assertIn(
            "task_added",
            EVENT_BUS._subscribers,
            "ChatModule should be subscribed to task_added",
        )
        self.assertIn(
            "settings_updated",
            EVENT_BUS._subscribers,
            "ChatModule should be subscribed to settings_updated",
        )
        self.assertIn(
            "agent_updated",
            EVENT_BUS._subscribers,
            "AgentsModule should be subscribed to agent_updated",
        )
        self.assertIn(
            "plugin_activated",
            EVENT_BUS._subscribers,
            "PluginsModule should be subscribed to plugin_activated",
        )
        self.assertIn(
            "stats_updated",
            EVENT_BUS._subscribers,
            "StatsModule should be subscribed to stats_updated",
        )
        self.assertIn(
            "system_event",
            EVENT_BUS._subscribers,
            "SystemControlModule should be subscribed to system_event",
        )
        self.assertIn(
            "improvement_update",
            EVENT_BUS._subscribers,
            "SelfImprovementCenter should be subscribed to improvement_update",
        )
    ...
    def test_event_publishing_with_no_subscribers(self):
        ...
        try:
            ...
            self.assertTrue(
                True, "Publishing with no subscribers should not raise an exception"
            )
    ...
    def test_event_handling_with_module_failure(self, mock_chat):
        ...
        self.assertTrue(
            True, "Event publishing should not crash with failed module initialization"
        )
