...
class PluginAdvancedFeaturesTest(unittest.TestCase):
    def setUp(self):
        ...
        self.plugin_manager = PluginManager(
            agent_manager=self.mock_agent_manager, plugin_dir="test_plugins"
        )
    ...
    def test_plugin_dependency_management(self):
        ...
        self.plugin_manager.plugins = {
            "OtherPlugin": {
                "metadata": {"name": "OtherPlugin", "version": "1.0.0"},
                "enabled": True,
            }
        }
        ...
        with (
            patch.object(
                ...
                return_value=plugin_metadata,
            ),
            patch.object(self.plugin_manager, "_check_dependency", return_value=True),
        ):
    ...
    def test_plugin_conflict_resolution(self):
        ...
        self.plugin_manager.plugins = {
            ...
            "SharedDep": {
                "metadata": {"name": "SharedDep", "version": "1.0.0"},
                "enabled": True,
            },
        }
        ...
        self.assertIn(
            "SharedDep", conflict_report, "Conflict should be detected for SharedDep"
        )
        self.assertEqual(
            len(conflict_report["SharedDep"]),
            1,
            "One conflict should be reported for SharedDep",
        )
        self.assertEqual(
            conflict_report["SharedDep"][0]["plugin"],
            "PluginA",
            "Conflict should be reported for PluginA",
        )
    ...
    def test_enable_plugin_with_dependencies(self):
        ...
        with patch.object(
            self.plugin_manager, "_check_dependencies", return_value=True
        ):
            result = self.plugin_manager.enable_plugin(plugin_name)
            self.assertTrue(
                result, "Plugin should be enabled when dependencies are met"
            )
            self.assertTrue(
                self.plugin_manager.plugins[plugin_name]["enabled"],
                "Plugin status should be updated to enabled",
            )
    ...
    def test_disable_plugin_with_dependents(self):
        ...
        self.assertFalse(
            result, "Plugin should not be disabled if there are active dependents"
        )
        self.assertTrue(
            self.plugin_manager.plugins[plugin_name]["enabled"],
            "Plugin status should remain enabled",
        )
