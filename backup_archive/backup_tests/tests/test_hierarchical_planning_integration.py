...
class TestHierarchicalPlanningIntegration:
    ...
    def test_full_planning_flow_success(self, mock_dependencies):
        ...
        master_agent.strategic_planner.generate_strategic_plan.return_value = (
            strategic_objectives
        )
        master_agent.tactical_planner.generate_tactical_plan.return_value = (
            tactical_plan
        )
        master_agent.operational_planner.generate_operational_plan.return_value = (
            operational_plan
        )
        ...
        with patch.object(
            ...
            master_agent.strategic_planner.generate_strategic_plan.assert_called_once_with(
                goal
            )
            master_agent.tactical_planner.generate_tactical_plan.assert_called_once_with(
            ...
            assert any(
                "Phase 2: Breaking down objective" in str(call) for call in calls
            )
            assert any(
                "Executing: Use web search to find papers." in str(call)
                for call in calls
            )
