...
def test_main_config_manager():
    ...
    for key, expected in test_keys.items():
        ...
        if actual == expected:
            ...
            print(
                f"    ❌ {key}: очікувався {expected[:10]}..., отримано {actual[:10]}..."
            )
            return False
...
def test_llm_manager_integration():
    ...
    try:
        ...
        config.set_setting(
            "openai_api_key", "sk-test-valid-openai-key-at-least-20-chars"
        )
        config.set_setting(
            "gemini_api_key", "AIzaTest-valid-gemini-key-at-least-20-chars"
        )
