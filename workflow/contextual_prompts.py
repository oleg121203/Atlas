...
class ContextualPromptGenerator:
    ...
    def generate_contextual_prompt(
        ...
        user_role = self.user_roles.get(
            user_id, self.user_roles.get("default_role", {})
        )
        ...
        if user_history:
            actions = [
                ...
                for step in h.get("workflow", {}).get("steps", [])
            ]
            action_counts = {
                action: actions.count(action) for action in set(actions) if action
            }
        ...
        self.dashboard_configs.get("metrics", {})
        ...
        if role_preferences:
            pref_str = ", ".join(
                [
                    ...
                    if k in ["style", "complexity", "domain"]
                ]
            )
            if pref_str:
        ...
        if common_actions:
            actions_str = ", ".join(
                [f"{action} ({count} times)" for action, count in common_actions]
            )
