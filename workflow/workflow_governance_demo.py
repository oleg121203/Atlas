...
from workflow_governance import AccessRole, ComplianceStandard, WorkflowGovernance
...
def run_demo():
    ...
    governance.request_approval(
        workflow_id, version, user_id, "Initial deployment request"
    )
    ...
    governance.approve_workflow(
        workflow_id, version, approver_id, "approved", "Approved for production"
    )
    ...
    compliance_result = governance.run_compliance_check(
        workflow_id, ComplianceStandard.GDPR, sample_workflow
    )
    ...
    governance.configure_role_access(
        workflow_id, AccessRole.ADMIN, ["view", "edit", "execute", "approve"]
    )
