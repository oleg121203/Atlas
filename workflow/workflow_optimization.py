...
class WorkflowOptimizer:
    ...
    def build_workflow_graph(self, workflow_id: str, days: int = 30) -> nx.DiGraph:
        ...
        for exec_id in executions:
            exec_steps = wf_steps[wf_steps["execution_id"] == exec_id].sort_values(
                "start_time"
            )
            for i in range(len(exec_steps) - 1):
                ...
                if G.has_edge(current_step, next_step):
                    ...
                    G[current_step][next_step]["duration"] += exec_steps.iloc[i][
                        "duration"
                    ]
                else:
                    G.add_edge(
                        ...
                        next_step,
                        weight=1,
                        duration=exec_steps.iloc[i]["duration"],
                    )
    ...
    def identify_critical_path(self, workflow_id: str) -> List[str]:
        ...
        critical_path = max(
            all_paths,
            key=lambda path: sum(
                G[path[i]][path[i + 1]]["avg_duration"] for i in range(len(path) - 1)
            ),
            default=[],
        )
    ...
    def cluster_executions(
        self, workflow_id: str, days: int = 30, n_clusters: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        if len(wf_executions) < n_clusters:
            print(
                f"Insufficient execution data for clustering in workflow {workflow_id}"
            )
    ...
    def recommend_optimizations(
        ...
        if critical_path:
            path_steps = [
                self.graphs[workflow_id].nodes[step]["name"] for step in critical_path
            ]
            total_duration = sum(
                self.graphs[workflow_id][critical_path[i]][critical_path[i + 1]][
                    "avg_duration"
                ]
        ...
        return (
            ...
            if recommendations
                ...
                {
                    "type": "no_issues",
                    "description": "No significant optimization opportunities identified at this time.",
                }
            ]
        )
    ...
    def evaluate_optimization_impact(
        ...
        before_metrics = self.analytics.get_performance_metrics(
            workflow_id, days=before_days + after_days
        )
        after_metrics = self.analytics.get_performance_metrics(
            workflow_id, days=after_days
        )
        ...
        return {
            ...
            "impact": {
                "success_rate_change": after_metrics["success_rate"]
                - before_metrics["success_rate"],
                "duration_change": after_metrics["average_duration"]
                - before_metrics["average_duration"],
            },
        }
