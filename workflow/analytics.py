...
class WorkflowAnalytics:
    ...
    def start_execution(self, execution_id: str, workflow_id: str) -> None:
        ...
        try:
            ...
            self.conn.commit()
            logger.info(f"Started monitoring execution {execution_id} for workflow {workflow_id}")
        except sqlite3.Error as e:
            logger.error(f"Failed to start monitoring execution {execution_id}: {e}")
            raise

    def end_execution(self, execution_id: str, status: str, details: Optional[str] = None) -> None:
        ...
        try:
            ...
            self.conn.commit()
            logger.info(f"Ended monitoring execution {execution_id} with status {status}")
        except sqlite3.Error as e:
            logger.error(f"Failed to end monitoring execution {execution_id}: {e}")
            raise

    def record_action(self, execution_id: str, action_name: str, start_time: datetime, end_time: datetime, status: str, error_message: Optional[str] = None) -> None:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (execution_id, action_name, start_time, end_time, duration, status, error_message or ''))
            ...
            self.conn.commit()
            logger.info(f"Recorded action {action_name} for execution {execution_id} with status {status}")
        except sqlite3.Error as e:
            logger.error(f"Failed to record action {action_name} for execution {execution_id}: {e}")
            raise

    def get_workflow_performance(self, workflow_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            columns = ['execution_id', 'start_time', 'end_time', 'duration_seconds', 'status', 'actions_count', 'failed_actions_count', 'details']
            results = [dict(zip(columns, row)) for row in cursor.fetchall()]
            logger.info(f"Retrieved performance data for workflow {workflow_id} with {len(results)} executions")
            return results
        except sqlite3.Error as e:
            logger.error(f"Failed to retrieve performance data for workflow {workflow_id}: {e}")
            raise
    ...
    def get_action_performance(self, execution_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            columns = ['id', 'action_name', 'start_time', 'end_time', 'duration_seconds', 'status', 'error_message']
            results = [dict(zip(columns, row)) for row in cursor.fetchall()]
            logger.info(f"Retrieved action performance data for execution {execution_id} with {len(results)} actions")
            return results
        except sqlite3.Error as e:
            logger.error(f"Failed to retrieve action performance data for execution {execution_id}: {e}")
            raise
        ...
        for action_name, error_message, count in failed_actions:
            if count > 1:  # Only generate recommendations for recurring failures
                ...
                if "timeout" in error_message.lower():
                    recommendations.append(f"Consider increasing timeout for action '{action_name}'")
                elif "permission" in error_message.lower() or "access denied" in error_message.lower():
                    recommendations.append(f"Check permissions for action '{action_name}'")
                elif "not found" in error_message.lower():
                    recommendations.append(f"Verify resource availability for action '{action_name}'")
    ...
    def optimize_workflow(self, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_duration_data and total_duration_data[0] is not None:
                ...
                if avg_total_duration > 10.0:  # Arbitrary threshold for long-running workflows
            ...
            logger.info(f"Generated optimization suggestions for workflow {workflow_id}")
            return suggestions
        except sqlite3.Error as e:
