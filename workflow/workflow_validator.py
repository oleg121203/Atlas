...
class WorkflowValidator:
    ...
    def validate_workflow(self, workflow: Dict[str, Any]) -> tuple[bool, List[str]]:
        ...
        for i, step in enumerate(workflow["steps"]):
            ...
            if not step_valid:
                is_valid = False
                errors.extend([f"Step {i+1} ({step.get('id', 'unknown')}): {err}" for err in step_errors])
            step_ids.add(step.get("id", f"step_{i}"))
            
        # Validate dependencies
        dependency_valid, dependency_errors = self._validate_dependencies(workflow["steps"], step_ids)
    ...
    def _validate_step(self, step: Dict[str, Any], existing_ids: set) -> tuple[bool, List[str]]:
    ...
    def _validate_dependencies(self, steps: List[Dict[str, Any]], step_ids: set) -> tuple[bool, List[str]]:
        ...
        if is_valid:
            ...
            for step in steps:
                ...
                if step_id not in visited:
                    cycle_detected = self._detect_cycle(step, steps, step_id, visited, path)
                    if cycle_detected:
    ...
    def _detect_cycle(self, step: Dict[str, Any], steps: List[Dict[str, Any]], step_id: str, visited: set, path: set) -> bool:
        ...
        for dep_id in step.get("dependencies", []):
            ...
            if dep_step and dep_id not in visited:
                if self._detect_cycle(dep_step, steps, dep_id, visited, path):
                    return True
